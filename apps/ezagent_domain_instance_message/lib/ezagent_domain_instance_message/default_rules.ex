defmodule EzagentDomainInstanceMessage.DefaultRules do
  ...
  defp pop_injected_fault(step) do
    faults = Application.get_env(:ezagent_domain_instance_message, :default_rules_migration_fault)
      ...
      list ->
        case Enum.split_with(list, fn {s, _err} -> s == step end) do
          ...
          {[{^step, error} | more_for_step], rest} ->
            ...
            if remaining == [] do
              Application.delete_env(:ezagent_domain_instance_message, :default_rules_migration_fault)
            else
              Application.put_env(:ezagent_domain_instance_message, :default_rules_migration_fault, remaining)
            end
