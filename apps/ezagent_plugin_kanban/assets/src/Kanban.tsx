      ...
      <div className="flex max-w-2xl flex-col gap-4 p-6">
        <div className="flex flex-col gap-3 rounded-md border border-border bg-card p-4" data-world-kanban-empty-create>
          <div>
            ...
            <p className="text-sm text-muted-foreground">这里还没有看板。建一块开始——建完自动进操作面（<strong>建议英文/数字/连字符命名</strong>，中文可能被拒）；或者点别人分享的看板链接，把它领进你自己的看板列表（只读钥匙发给你本人）。</p>
          </div>
        ...
        <div className="flex flex-shrink-0 items-center gap-1.5">
          ...
          {state.instances?.some((i) => i.uri === uri && !i.owned) && (
            <Button
              ...
              size="sm"
              variant="secondary"
              title="向板主人申请编辑权限（只读 → 可编辑）"
              data-world-kanban-request-edit
              onClick={() => onAction("kanban.request_edit", {kanban_uri: uri})}
            >
              申请编辑
            </Button>
          )}
          ...
          {(node.artifacts ?? []).map((raw, i) => {
            ...
            return (
              <li key={i} className="flex items-center gap-1.5">
                ...
                {/* ㊲ 清算（2026-07-20）：file 附件的 url 即渲染期预签的 person-bound
                    下载 href（main #1471——token 绑读板者本人，serve 按 caller==grantee
                    判权），直接 <a> 打开即下载；点击现签 dispatch 权宜已移除。
                    ㊳ 链接等其余产物：绝对 URL 原样新窗打开，裸域名补 https://，不拼站点 base。 */}
                {a.url && (
                  <a
                    href={artifactOpenHref(a.url)}
                    target="_blank"
                    rel="noreferrer"
                    className="text-primary hover:underline"
                    {...(a.kind === "file" ? {"data-world-kanban-artifact-download": true} : {})}
                  >
                    打开
                  </a>
                )}
