defmodule Ezagent.Behavior.Kanban.Shared do
  ...
  # 任何具体棒名。两条路径：
  #
  #   * 生产：dispatch ctx 带 `:self_uri`（agent URI）→ `AgentRoleAttributes.fetch/1`
  ...
  # role 名：ETS 快径（spawn 时 role_step.grant_role_marker 写入）→ durable 快照兜底。
  # 两者都无（无 self_uri / 未起活）→ :none，调用方退 default。
  defp role_of(%URI{} = uri) do
    case Ezagent.AgentRoleAttributes.fetch(uri) do
      {:ok, role_name} -> {:ok, role_name}
      :none -> Ezagent.AgentRoleResolver.role_from_durable_snapshot(uri)
    end
  end
