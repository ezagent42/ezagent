defmodule Ezagent.ActionSet.KanbanTest do
  ...
  **不变式**：`owner==nil ⟺ status==:unassigned`。
  ...
  # 棒链来自 recipe config 数据（layer-2）——测试经此数据路径注入 ctx。
  ...
  # wildcard admin ctx，带 caller（加节点自动认领需 caller）。
  defp admin_ctx(tree),
    do:
      Map.merge(@recipe_config, %{
        read: rd(tree),
        caps: MapSet.new([@admin_cap]),
        caller: Ezagent.URI.new!(@admin)
      })
  ...
  # 版主 ctx：非 wildcard，但 caller == 本板 data_owner（经 :data_owner 直注，测试夹具）。
  ...
  defp no_caller_ctx(tree),
    do: Map.merge(@recipe_config, %{read: rd(tree), caps: MapSet.new([@admin_cap])})
