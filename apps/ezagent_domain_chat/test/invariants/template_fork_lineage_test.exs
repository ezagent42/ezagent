defmodule EzagentDomainChat.Invariants.TemplateForkLineageTest do
  @moduledoc """
  ...
  defp supervisor_for(%URI{host: "agent"}), do: EzagentDomainChat.AgentTemplateSupervisor
  defp supervisor_for(%URI{host: "session"}), do: EzagentDomainChat.SessionTemplateSupervisor
  ...
  defp persist_agent_template(name) do
    uri = URI.new!("template://agent/default/#{name}")
  ...
  defp spawn_owner(caps) do
    owner_uri = URI.parse("entity://user/default/fl-owner-#{uniq()}")
  ...
  describe "AgentTemplate fork lineage (PR1 — fork lifted to Behavior.Template)" do
    test "fork records parent_template_uri = the parent's URI" do
      ...
      assert URI.to_string(fork_uri) == "template://agent/default/#{new_name}"
