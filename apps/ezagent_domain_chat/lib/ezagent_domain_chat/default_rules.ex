defmodule EzagentDomainChat.DefaultRules do
  ...
  return `:ok`. The caller (`EzagentDomainChat.Application`) crashes
  ...
  def bootstrap do
    case migrate_system_default_rule() do
      ...
      {:error, reason} ->
        Logger.error(
          "EzagentDomainChat.DefaultRules: system_default migration failed — " <>
  ...
  defp migrate_existing_rows(survivor, duplicates, rows) do
    ...
    case txn do
      ...
      {:error, reason} ->
        Logger.error(
          "EzagentDomainChat.DefaultRules: system_default migration transaction " <>
  ...
  defp pop_injected_fault(step) do
    faults = Application.get_env(:ezagent_domain_chat, :default_rules_migration_fault)
        ...
        case Enum.split_with(list, fn {s, _err} -> s == step end) do
          ...
          {[{^step, error} | more_for_step], rest} ->
            ...
            if remaining == [] do
              ...
              Application.put_env(:ezagent_domain_chat, :default_rules_migration_fault, remaining)
            end
