defmodule Ezagent.Entity.Session do
  ...
  def ensure_orchestrator(
    ...
    # (`EzagentDomainChat.create_session/3`) rolls back on `{:error, _}`.
  ...
  public so the atomic `EzagentDomainChat.create_session/3` can merge
  ...
  lives in `EzagentDomainChat.create_session/3`, the single chokepoint,
  ...
  the atomic `EzagentDomainChat.create_session/3` chokepoint can call it
  ...
  `EzagentDomainChat.create_session/3` can demand-spawn a SessionTemplate
  ...
  2026-05-31 orchestrator-startup-atomicity §4 — kept + made public so
  the atomic `EzagentDomainChat.create_session/3` can read the template's
