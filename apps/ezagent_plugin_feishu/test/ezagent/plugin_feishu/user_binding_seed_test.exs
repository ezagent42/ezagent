defmodule EzagentPluginFeishu.UserBindingSeedTest do
  ...
  setup do
    ...
    Application.put_env(:ezagent_plugin_feishu, :seed_executor, EzagentPluginFeishu.UserBindingSeed.FakeExecutor)
    on_exit(fn ->
      Application.delete_env(:ezagent_plugin_feishu, :seed_enabled)
      Application.delete_env(:ezagent_plugin_feishu, :seed_executor)
      EzagentPluginFeishu.UserBindingSeed.FakeExecutor.stop()
    end)
    EzagentPluginFeishu.UserBindingSeed.FakeExecutor.start()
    :ok
  end
  ...
  describe "missing / invalid file" do
    test "missing file is a normal no-op" do
      assert {:ok, :absent} = UserBindingSeed.run("/nonexistent/#{System.unique_integer([:positive])}/bindings.yaml")
    end
    ...
    test "a present-but-invalid row fails loud; no binds planned" do
      ...
      assert EzagentPluginFeishu.UserBindingSeed.FakeExecutor.bind_count() == 0
    end
  end
  ...
  describe "absent → bind planned" do
    test "absent rows → structured summary, bind recorded" do
      ...
      assert EzagentPluginFeishu.UserBindingSeed.FakeExecutor.bind_count() == 1
    end
    ...
    test "multiple absent rows → all planned" do
      ...
      assert EzagentPluginFeishu.UserBindingSeed.FakeExecutor.bind_count() == 2
    end
  end
  ...
  describe "same → pure no-op, no bind planned" do
    test "existing binding → classified :same, zero binds" do
      ...
      EzagentPluginFeishu.UserBindingSeed.FakeExecutor.stop()
      EzagentPluginFeishu.UserBindingSeed.FakeExecutor.start(existing: [%{open_id: open_id, user_uri: uri_str}])
      Application.put_env(:ezagent_plugin_feishu, :seed_executor, EzagentPluginFeishu.UserBindingSeed.FakeExecutor)
      yaml = """
      bindings:
        - open_id: "#{open_id}"
  ...
  describe "conflict → whole-file gate, zero binds" do
    test "existing open_id with DIFFERENT user_uri → conflict" do
      ...
      EzagentPluginFeishu.UserBindingSeed.FakeExecutor.stop()
      EzagentPluginFeishu.UserBindingSeed.FakeExecutor.start(existing: [%{open_id: open_id, user_uri: "entity://team-alpha/user/bob"}])
      Application.put_env(:ezagent_plugin_feishu, :seed_executor, EzagentPluginFeishu.UserBindingSeed.FakeExecutor)
      yaml = """
      bindings:
        - open_id: "#{open_id}"
