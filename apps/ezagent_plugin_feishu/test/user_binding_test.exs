defmodule EzagentPluginFeishu.UserBindingTest do
  ...
  test "bind then resolve" do
    ...
    {:ok, _row} = UserBinding.bind(open_id, "entity://user/team-alpha/alice", "entity://user/system/admin")

    assert {:ok, %URI{} = uri} = UserBinding.resolve(open_id)
    assert URI.to_string(uri) == "entity://user/team-alpha/alice"
  end
  ...
  test "rebind replaces user_uri silently" do
    ...
    {:ok, _} = UserBinding.bind(open_id, "entity://user/team-alpha/alice", "entity://user/system/admin")
    {:ok, _} = UserBinding.bind(open_id, "entity://user/team-alpha/bob", "entity://user/system/admin")

    {:ok, uri} = UserBinding.resolve(open_id)
    assert URI.to_string(uri) == "entity://user/team-alpha/bob"
  end
  ...
  test "open_ids_for finds all bindings for a user" do
    user = "entity://user/team-alpha/multi_#{System.unique_integer([:positive])}"
