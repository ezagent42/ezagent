defmodule EzagentPluginAutoservice.OperatorLive do
  ...
  def handle_event("select", %{"uri" => uri_str}, socket) do
    ...
    # Explicit human handoff: flip the session into :takeover so the AI
    # stops auto-replying to the customer while the operator drives the
    # conversation (Behavior.Mode, PR-A). Idempotent (re-selecting an
    # already-taken-over session is a no-op flip).
    set_mode(session_uri, :takeover, socket.assigns.operator_uri, socket.assigns.caps)
  ...
  def handle_event("resume_ai", _params, socket) do
    case socket.assigns.selected do
      nil ->
        {:noreply, socket}

      %URI{} = session_uri ->
        # Hand the conversation back to the AI: flip the session into
        # :auto so Chat.send resumes the agent auto-reply fan-out.
        set_mode(session_uri, :auto, socket.assigns.operator_uri, socket.assigns.caps)
        {:noreply, socket}
    end
  end
  ...
  # Flip the session's operator-control mode via Behavior.Mode (PR-A).
  # Mirrors join_operator/3's dispatch shape (?action=<behavior>.<action>,
  # :cast, reply: :ignore) for consistency. `:takeover` suppresses agent
  # auto-replies; `:auto` resumes them.
  defp set_mode(session_uri, mode, operator_uri, caps) when mode in [:auto, :takeover] do
      ...
      <main class="flex-1 flex flex-col rounded-xl border border-gray-200 bg-white overflow-hidden">
        ...
        <%= if @selected do %>
          <header class="px-4 py-3 border-b bg-gray-50 flex items-center justify-between">
            ...
            <button
              phx-click="resume_ai"
              class="text-xs px-3 py-1.5 rounded-lg border border-gray-300 bg-white hover:bg-gray-100 text-gray-700"
            >
              交还 AI
            </button>
          </header>
