defmodule EzagentPluginAutoservice.CustomerLive do
  ...
  Ported verbatim from Stage-1 (#715) with two MINIMAL changes:
  1. `SocialwareCS.ensure_joined` → `Assembly.ensure_joined` (the old
     GenServer-backed SocialwareCS module is gone; Assembly is the
     idempotent provisioner on this branch).
  2. `SocialwareCS.ensure_adapter` call REMOVED — there is no per-node
     adapter anymore; the CsOrchestrator Kind is provisioned at seed.
  """
  ...
  def mount(_params, _session, socket) do
    ...
    case Assembly.ensure_joined(customer_uri) do
      {:ok, session_uri} ->
