  ...
  **工作目录(cwd)** 并放开 `Read/Glob/LS` —— 它能像平时一样 `Read zuatu-source-backup/src/App.jsx`
  ...
  def render_for_builder(ws, sid) do
    ...
    if items == [] do
      ...
      下面是清单(已含**完整相对路径**,文件可能在子目录里)。**需要某个素材的实际内容时,用
      `Read <相对路径>` 工具读**,不要凭空猜。
      ⚠️ 用户点名某个文件时**常只给文件名或部分路径**(可能漏掉子目录前缀,或写成 `/xxx`)——请在
      下面清单里**按文件名匹配到那条完整相对路径再 `Read`**;清单里找不到再用 `Glob **/<文件名>` 定位。
      图片素材用上面给的 `/loom/materials/...` URL 在页面里 `<img src>` 引用。
      #{Enum.join(lines, "\n")}
      """
    end
  end
