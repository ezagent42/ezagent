defmodule Ezagent.Cap.ShareTokenTest do
  ...
  describe "mint_link!/4 + verify_link/1" do
    test "round-trips the exact target + behavior + actions" do
      token = ShareToken.mint_link!(@board, @actionset, @read_actions, ttl_seconds: 60)

      assert {:ok, %{target: target, behavior: behavior, actions: actions}} =
    ...
    test "is URI-agnostic — any target scheme mints + verifies (not uploads-locked)" do
      for target <- [
        ...
        token = ShareToken.mint_link!(target, @actionset, @read_actions, ttl_seconds: 60)
    ...
    test "non-positive / over-ceiling TTL is rejected at mint" do
      assert_raise ArgumentError, fn ->
        ShareToken.mint_link!(@board, @actionset, @read_actions, ttl_seconds: 0)
      end

      assert_raise ArgumentError, fn ->
        ShareToken.mint_link!(@board, @actionset, @read_actions, ttl_seconds: 2_592_001)
      end
    end

    test "empty actions is rejected at mint (a link must carry an intent)" do
      assert_raise ArgumentError, fn ->
        ShareToken.mint_link!(@board, @actionset, [], ttl_seconds: 60)
      end
    end
