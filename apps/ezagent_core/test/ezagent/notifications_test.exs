defmodule Ezagent.NotificationsTest do
  ...
  defp unique_user_uri(suffix),
    do:
      Ezagent.URI.new!(
        "entity://team-alpha/user/notif_test_#{suffix}_#{System.unique_integer([:positive])}"
      )
  ...
  describe "topic/1" do
    test "wraps URI in esr:user:...:events" do
      ...
      assert Notifications.topic(uri) == "esr:user:entity://team-alpha/user/topic_test:events"
    end
  end
  ...
  describe "subscribe_slice_change/1 — PR-N2 transition helper" do
    ...
    test "delegates topic to Ezagent.SliceChange.topic/1 (no shape drift)" do
      uri = Ezagent.URI.new!("entity://team-alpha/user/topic_delegate_test")
