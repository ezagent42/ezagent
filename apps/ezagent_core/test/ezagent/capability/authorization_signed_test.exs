defmodule Ezagent.Capability.AuthorizationSignedTest do
  @moduledoc """
  Unified-revocation F-2: the shared preflight predicate must reject a
  target-signed capability after the target generation changes.
  """
  ...
  test "authorizes? denies a capability whose target generation was bumped" do
    ...
    assert Capability.Authorization.authorizes?([cap], needed)
    assert {:ok, _bumped} = Authority.regenesis(target, :test, admin())
    refute Capability.Authorization.authorizes?([cap], needed)
  end
  ...
  defp unique_uri(suffix) do
    Ezagent.URI.new!(
      "entity://team-alpha/agent/f2-preflight-#{suffix}-#{System.unique_integer([:positive])}"
    )
  end

  defp unique_user(suffix) do
    Ezagent.URI.new!(
      "entity://team-alpha/user/f2-preflight-#{suffix}-#{System.unique_integer([:positive])}"
    )
  end
