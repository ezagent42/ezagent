...
# (no collector process, no handler, no dump file). The collector module
# lives in the sibling ezagent_actor app, loaded only in umbrella runs —
# skipped standalone. The dump feeds docs/notes/v5-use-side-ingress-census.md;
# it NEVER gates CI.
if System.get_env("EZAGENT_INGRESS_CENSUS") == "1" and
     Code.ensure_loaded?(Ezagent.Kind.IngressCensus.Collector) do
  _ = Ezagent.Kind.IngressCensus.Collector.attach()

  ExUnit.after_suite(fn _result ->
    path = Path.expand("../../../_build/census/ezagent_core_ingress_census.txt", __DIR__)
