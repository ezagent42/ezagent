defmodule EzagentCore.Invariants.NoRoleConceptInCoreTest do
  ...
  `Ezagent.Agent.RecipeRegistry` resolves roles READ-THROUGH over
  ...
  re-introduced a `RecipeRegistry` reference into `ezagent_core/lib`, core would
  ...
  references a `RecipeRegistry` module (in ANY namespace —
  `Ezagent.Agent.RecipeRegistry`, a bare aliased `RecipeRegistry`, etc.).

  ## What is ALLOWED in core (NOT flagged)

  - `Ezagent.Agent.Recipe` / `Ezagent.Agent.Recipe.Compose` / `Ezagent.Agent.Recipe.CapMint` — the role
  ...
  - `Ezagent.Plugin.RoleSeedHook` — core's OWN registered-hook seam (the
    ...
    registry). It is defined in core; its name does not end in `:RecipeRegistry`.

  ## Why AST, not grep

  The scan walks the parsed AST for `__aliases__` nodes whose LAST segment is
  `RecipeRegistry`. Comments and docstrings that merely MENTION the module (several
  ...
  test "ezagent_core/lib has zero RecipeRegistry module references" do
    offending =
      core_lib_files()
    ...
    assert offending == [],
           """
           ezagent_core/lib must not reference a RecipeRegistry module.

           The role registry (`Ezagent.Agent.RecipeRegistry`) resolves over
    ...
    {_ast, refs} =
      Macro.prewalk(ast, [], fn
        {:__aliases__, meta, atoms} = node, refs when is_list(atoms) ->
          if List.last(atoms) == :RecipeRegistry do
