defmodule Mix.Tasks.Ezagent.CheckInvariants do
  ...
  defp check_invariant_1 do
    ...
    #   (2026-07-13: `auth_observers.ex` / `phase_broadcast.ex` /
    #   `respawn_policy.ex` carry PTY fan-out that was already allowlisted
    #   inside `server.ex` — extracted verbatim to keep that module under the
    #   oversized-module gate — plus the new respawn-halt signal. Same
    #   category: operator-visibility fan-out, never an inbound message.)
      ...
      System.cmd(
        ...
        [
          ...
          "grep -rnE 'PubSub\\.broadcast' apps 2>/dev/null --include='*.ex' " <>
            ...
            "| grep -v 'apps/ezagent_domain_pty/lib/ezagent_domain_pty/respawn_policy.ex' " <>
