defmodule Ezagent.Plugin do
  ...
  # NOTE (2026-06-30): `nav_surfaces/0` + `session_tabs/0` are NO LONGER core
  # `Ezagent.Plugin` callbacks. They are World-UI concepts (left-rail nav +
  # per-session conversation tab) whose only consumer is the world shell, so
  # the contract moved OUT of core to `Ezagent.World.UiSurfaceProvider`
  # (`apps/ezagent_plugin_world`). A plugin contributes a surface by defining a
  # plain `nav_surfaces/0` / `session_tabs/0` function (duck-typed); world reads
  # it via `function_exported?/3` + read-time shape filtering. `config_surface/0`
