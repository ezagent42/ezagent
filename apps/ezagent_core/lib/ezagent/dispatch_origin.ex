defmodule Ezagent.DispatchOrigin do
  ...
  `:authenticated_external` means `ctx.caller` was overwritten from an
  authenticated session or bearer at the transport edge. `:trusted_internal`
  identifies reviewed framework code. An absent or unknown stamp fails before
  ...
  def validate(:authenticated_external, %{caller: %URI{} = presenter} = ctx) do
