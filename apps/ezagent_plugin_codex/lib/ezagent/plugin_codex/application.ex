defmodule EzagentPluginCodex.Application do
  ...
  @impl Ezagent.Plugin
  def children do
    # V5 pid-closure A1b: the private `AppServerRegistry` /
    # `BridgeSidecarRegistry` are RETIRED — both sidecars self-register in
    # the unified `Ezagent.Runtime.SidecarRegistry` (started by
    # `EzagentActor.Application`) and are reached only through the
    # `Ezagent.Runtime.Resolver` seam. The supervisors stay: spawn remains
    # the plugin's own `DynamicSupervisor.start_child`.
    [
