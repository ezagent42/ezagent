defmodule Ezagent.Kind.Server do
  ...
  # raise/exit is converted to `{:error, _}` so init/1 can `{:stop, ...}` cleanly
  # rather than abort with an opaque `:EXIT` tuple — symmetric with the durable
  # `save_now/4` catch above.
