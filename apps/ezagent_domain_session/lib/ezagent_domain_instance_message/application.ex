defmodule EzagentDomainInstanceMessage.Application do
  ...
  defp register_spawn_fns do
    ...
    # agent flavors.
    :ok =
      Ezagent.SpawnRegistry.register("entity", fn uri, opts ->
  ...
  defp spawn_agent(%URI{} = uri, opts) do
    case AgentModuleResolver.lookup_kind_module_for_agent(uri) do
      {:ok, kind_module} ->
        ...
        # derivation-edge: rehydration-only — persisted agent row owns its edge
        Ezagent.Kind.spawn(kind_module, %{uri: uri}, opts)
