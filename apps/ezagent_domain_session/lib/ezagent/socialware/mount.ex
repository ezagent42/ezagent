defmodule Ezagent.Socialware.Mount do
  ...
  **零 kanban 字面**:behavior / role / actions / access 全由调用方参数化。拉板传全动作
  ...
  @doc """
  卸载一个 target(数据宿主)名下的**全部**挂载行(跨 session、跨 grantee,含 person
  行)—— 宿主被删除/退休时的挂载清理。逐行卸载(撤钥匙 + 删行;session 行走
  `unmount/4`,person 行走 `unmount_for_person/3`);单行失败只记 `:warning` 计入
  `failed`,不牵连其余行(与 `reconcile_session_mounts/1` 同款 best-effort 姿态)。
  ...
  # person 行无 session 轴(`session_uri` 为 NULL)→ 走人本位卸载路;其余走 session 路。
  defp unmount_row(%MountRow{scope: "person"} = row) do
    unmount_for_person(
      Ezagent.URI.new!(row.target_uri),
      Ezagent.URI.new!(row.grantee_uri),
      decode_behavior(row.behavior)
    )
  end

  defp unmount_row(%MountRow{} = row) do
    unmount(
      Ezagent.URI.new!(row.session_uri),
      Ezagent.URI.new!(row.target_uri),
      Ezagent.URI.new!(row.grantee_uri),
      decode_behavior(row.behavior)
    )
