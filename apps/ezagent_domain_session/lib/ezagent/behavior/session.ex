    ...
    reconciled_members =
      case ctx do
        %{self_uri: %URI{} = session_uri} ->
          ...
          # a redundant second-truth-source pass and has been removed.
  ...
  def handle_send(%{message: %Message{} = msg}, ctx) do
    ...
    case MessageStore.write(msg, session_uri) do
      {:ok, stored_msg} ->
        ...
        working_copy = ctx[:read].(:template_working_copy, default_template_working_copy())
        ingress = Map.get(working_copy, :ingress)
        protected_roles = if ingress, do: Map.get(ingress, :protected_roles, []), else: []

        with :ok <- Delivery.dispatch_declared_ingress(session_uri, stored_msg, ingress) do
          ...
          if msg.hops <= 0 do
            ...
            resolved_recipients = Enum.map(recipients_with_ctx, fn {uri, _ctx} -> uri end)

            recipients_with_ctx =
              Delivery.exclude_protected_role_recipients(
                recipients_with_ctx,
                members_map,
                protected_roles
              )
            ...
            Delivery.notify_dropped_mentions(
              msg,
              resolved_recipients,
              session_uri,
              ctx,
          ...
          {:error, reason} -> {:error, {:ingress_dispatch_failed, reason}}
        end
