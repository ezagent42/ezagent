defmodule Ezagent.Behavior.EchoMigrationParityTest do
  ...
  describe ":receive parity" do
    test "produces a chat.send dispatch with text='echo: <input>'" do
      input_msg =
        Ezagent.Message.new(
          Ezagent.URI.new!("entity://ws/user/alice"),
          %{text: "ping"}
        )

      session_uri = Ezagent.URI.new!("session://ws/default/main")
      self_uri = Ezagent.URI.new!("entity://ws/agent/echo_test")
    ...
    test "increments count + stashes last_msg on :receive" do
      input_msg =
        Ezagent.Message.new(Ezagent.URI.new!("entity://ws/user/alice"), %{text: "pong"})
