defmodule EzagentPluginLiveview.AdminLive do
  ...
  #
  # Subscribe loop (mount/3) deliberately still uses the unfiltered
  # `SessionContext.list_sessions_for/1` so the LV inbox stays scoped to
  ...
  defp list_visible_sessions_for(%URI{scheme: "workspace", host: ws_name} = workspace_uri) do
    ...
    workspace_uri
    |> SessionContext.list_sessions_for()
    |> Enum.filter(fn uri ->
