defmodule EzagentPluginLiveview.AutoService.CustomerLive do
  ...
  def mount(_params, _session, socket) do
    ...
    case CustomerSession.ensure_joined(customer_uri) do
      {:ok, session_uri} ->
        ...
        # Customer token for the gated CustomerFeed replay (settled operator/bot
        # deliveries). Without it `CustomerFeed.replay(_, nil, _)` returns
        # :unauthorized and live deliveries (e.g. an operator's settled reply)
        # never render — the customer would only see them on a full reload.
  ...
  def handle_info({:customer_delivery, %{message_ids: _ids}}, socket) do
    ...
    if is_nil(session_uri) do
      ...
      # Replay the gated feed with the customer token issued at mount (a nil
      # token fails the CustomerAuth gate → settled operator/bot deliveries
      # would never render live).
  ...
  #
  # Defensive visibility gate (belt-and-suspenders): `Chat.handle_send`
  # broadcasts `{:chat_message}` UNCONDITIONALLY, with no visibility filter. The
  # operator-takeover reply is gated through the Turn (operator_only -> settle ->
  # customer_visible) and is NEVER chat.send'd, so it cannot arrive here — but if
  # any path ever broadcasts a non-`customer_visible` message into this session,
  # render ONLY `customer_visible` so a draft can never leak to the customer.
  # Bot replies are `customer_visible`, so they are unaffected.
