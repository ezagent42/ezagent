    ...
    uploaded =
      consume_uploaded_entries(socket, :kb_file, fn meta, _socket ->
        case KbStore.ingest_file(kb_dir, meta.path) do
          ...
          {:error, reason} -> {:postpone, "上传 #{meta.client_name} 失败: #{inspect(reason)}"}
        end
      end)
  ...
  def render(assigns) do
    ...
    <div class="max-w-6xl mx-auto p-6">
      <div class="flex items-center justify-between mb-4">
        <div>
          <h1 class="text-xl font-bold text-gray-900">KB 知识库管理</h1>
      ...
      <%= if @rebuild_flash do %>
        <div class={[
          ...
          String.contains?(@rebuild_flash, "成功") && "text-green-700 bg-green-50",
          String.contains?(@rebuild_flash, "失败") && "text-red-700 bg-red-50"
        ]}>
          {@rebuild_flash}
        </div>
      <% end %>
      ...
      <div class="flex gap-0.5 mb-0">
        <button
          ...
          class={[
            ...
            @tab == "sources" && "bg-gray-800 text-white",
            @tab != "sources" && "bg-gray-100 text-gray-600 hover:bg-gray-200"
          ]}
        >
          Sources
        </button>
        <button
          ...
          class={[
            "px-4 py-2 text-sm font-medium rounded-t-lg transition-colors",
            @tab == "url" && "bg-gray-800 text-white",
            @tab != "url" && "bg-gray-100 text-gray-600 hover:bg-gray-200"
          ]}
        >
          URL抓取
        </button>
        <button
          ...
          class={[
            "px-4 py-2 text-sm font-medium rounded-t-lg transition-colors",
            @tab == "upload" && "bg-gray-800 text-white",
            @tab != "upload" && "bg-gray-100 text-gray-600 hover:bg-gray-200"
          ]}
        >
          文件上传
        </button>
        <button
          ...
          class={[
            "px-4 py-2 text-sm font-medium rounded-t-lg transition-colors",
            @tab == "manual" && "bg-gray-800 text-white",
            @tab != "manual" && "bg-gray-100 text-gray-600 hover:bg-gray-200"
          ]}
        >
          手动添加
        </button>
        <button
          ...
          class={[
            "px-4 py-2 text-sm font-medium rounded-t-lg transition-colors",
            @tab == "glossary" && "bg-gray-800 text-white",
            @tab != "glossary" && "bg-gray-100 text-gray-600 hover:bg-gray-200"
          ]}
        >
          Glossary
        </button>
          ...
          <div class="overflow-x-auto">
            <%= if @sources == [] do %>
              <div class="px-4 py-8 text-center">
                <p class="text-sm text-gray-400">暂无 KB 数据源</p>
                <p class="text-xs text-gray-300 mt-1">通过 URL抓取、文件上传或手动添加来创建条目</p>
              </div>
            <% else %>
              <table class="w-full text-sm">
                <thead>
                  <tr class="border-b border-gray-200 bg-gray-50">
                    <th class="text-left px-4 py-2 font-medium text-gray-600 text-xs">类型</th>
                    <th class="text-left px-4 py-2 font-medium text-gray-600 text-xs">来源</th>
                    <th class="text-left px-4 py-2 font-medium text-gray-600 text-xs">标题</th>
                    <th class="text-right px-4 py-2 font-medium text-gray-600 text-xs">分块数</th>
                    <th class="text-right px-4 py-2 font-medium text-gray-600 text-xs">操作</th>
                  </tr>
                </thead>
                <tbody>
                  <%= for src <- @sources do %>
                    <tr class="border-b border-gray-100 hover:bg-gray-50">
                      <td class="px-4 py-2">
                        <span class="text-[10px] bg-gray-100 px-1.5 py-0.5 rounded text-gray-500 font-mono">
                          {src.source_type}
                        </span>
                      </td>
                      <td
                        class="px-4 py-2 text-xs text-gray-500 font-mono max-w-[200px] truncate"
                        title={src.source_id}
                      >
                        {src.source_id}
                      </td>
                      <td
                        class="px-4 py-2 text-xs text-gray-700 max-w-[250px] truncate"
                        title={src.title}
                      >
                        {src.title}
                      </td>
                      <td class="px-4 py-2 text-right text-xs text-gray-500">{src.chunk_count}</td>
      ...
      <%= if @tab == "url" do %>
        <div class="rounded-xl border border-gray-200 bg-white overflow-hidden rounded-tl-none">
          ...
          <div class="p-4 space-y-3">
            <p class="text-xs text-gray-500">输入 URL 抓取网页内容作为 KB 条目。HTML 标签会被自动剥离，最多保留 5000 字符。</p>
            <%= if @url_flash do %>
              <div class={[
                "text-xs rounded px-3 py-1.5",
                String.contains?(@url_flash, "成功") && "text-green-700 bg-green-50",
                String.contains?(@url_flash, "失败") && "text-red-700 bg-red-50"
              ]}>
                {@url_flash}
              </div>
            <% end %>
        ...
        <div class="rounded-xl border border-gray-200 bg-white overflow-hidden rounded-tl-none">
          ...
          <div class="p-4 space-y-3">
            ...
            <%= if @upload_flash do %>
              <div class={[
                "text-xs rounded px-3 py-1.5",
                String.contains?(@upload_flash, "失败") && "text-red-700 bg-red-50",
                !String.contains?(@upload_flash, "失败") && "text-green-700 bg-green-50"
              ]}>
                {@upload_flash}
              </div>
            <% end %>
            ...
            <form phx-submit="upload_files" phx-change="validate_upload">
              ...
              <%= for entry <- @uploads.kb_file.entries do %>
                <div class="flex items-center gap-2 mt-2 text-sm">
                  ...
                  <span class="text-xs text-gray-400">{round(entry.client_size / 1024)} KB</span>
              ...
              <%= for err <- upload_errors(@uploads.kb_file) do %>
                <div class="text-xs text-red-600 mt-1">{upload_error_to_string(err)}</div>
              <% end %>
      ...
      <%= if @tab == "manual" do %>
        <div class="rounded-xl border border-gray-200 bg-white overflow-hidden rounded-tl-none">
          ...
          <div class="p-4">
            <%= if @manual_flash do %>
              <div class="text-xs text-green-700 bg-green-50 rounded px-3 py-1.5 mb-3">
                {@manual_flash}
              </div>
            <% end %>
            <form phx-submit="manual_add" class="space-y-3">
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="text-xs font-medium text-gray-700">条目 ID *</label>
                  <input
                    type="text"
                    name="id"
                    value={@manual_id}
                ...
                <div>
                  <label class="text-xs font-medium text-gray-700">标题 *</label>
                  <input
                    type="text"
                    name="title"
                    value={@manual_title}
              ...
              <div>
                <label class="text-xs font-medium text-gray-700">内容 *</label>
                <textarea
                  name="content"
                  rows="6"
                  required
        ...
        <div class="rounded-xl border border-gray-200 bg-white overflow-hidden rounded-tl-none">
          ...
          <div class="p-4 space-y-4">
            <%= if @glossary_flash do %>
              <div class="text-xs text-green-700 bg-green-50 rounded px-3 py-1.5">
                {@glossary_flash}
              </div>
            <% end %>
            ...
            <%!-- Add / Edit form --%>
            <form
              phx-submit={if @glossary_edit_idx != nil, do: "glossary_update", else: "glossary_add"}
              class="space-y-2 p-3 bg-gray-50 rounded-lg border border-gray-200"
            >
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="text-xs font-medium text-gray-700">术语</label>
                  <input
                    type="text"
                    name="term"
                    value={@glossary_term}
                ...
                <div>
                  <label class="text-xs font-medium text-gray-700">定义</label>
                  <input
                    type="text"
                    name="definition"
              ...
              <div class="flex justify-end gap-2">
                <%= if @glossary_edit_idx != nil do %>
                  <button
                    ...
                    class="rounded border border-gray-300 text-gray-700 px-3 py-1.5 text-sm hover:bg-gray-50"
                  >
                    取消
                  </button>
                <% end %>
            ...
            <%= if @glossary != nil && map_size(@glossary) > 0 do %>
              <div class="overflow-x-auto">
                <table class="w-full text-sm">
                  <thead>
                    <tr class="border-b border-gray-200 bg-gray-50">
                      <th class="text-left px-4 py-2 font-medium text-gray-600 text-xs w-1/3">术语</th>
                      <th class="text-left px-4 py-2 font-medium text-gray-600 text-xs">定义</th>
                      <th class="text-right px-4 py-2 font-medium text-gray-600 text-xs w-24">操作</th>
                    </tr>
                  </thead>
                  <tbody>
                    <%= for {{term, definition}, idx} <- @glossary |> Map.to_list() |> Enum.with_index() do %>
                      <tr class="border-b border-gray-100 hover:bg-gray-50">
                        <td class="px-4 py-2 text-xs font-mono text-gray-800">{term}</td>
                        <td class="px-4 py-2 text-xs text-gray-600">{definition}</td>
                        <td class="px-4 py-2 text-right">
            ...
            <% else %>
              <div class="px-4 py-6 text-center">
                <p class="text-sm text-gray-400">暂无术语</p>
                <p class="text-xs text-gray-300 mt-1">使用上方表单添加术语定义</p>
              </div>
            <% end %>
          </div>
        </div>
