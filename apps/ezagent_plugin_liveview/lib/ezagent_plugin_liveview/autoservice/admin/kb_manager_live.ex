    ...
    <div class="flex min-h-screen">
      ...
      <%= if @rebuild_flash do %>
        <div class={[
          ...
          String.contains?(@rebuild_flash, "成功") && "text-green-700 dark:text-green-300 bg-green-50 dark:bg-green-950",
          String.contains?(@rebuild_flash, "失败") && "text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-950"
        ]}>
          {@rebuild_flash}
        </div>
      <% end %>
      ...
      <div class="flex gap-0.5 mb-0">
        <button
          ...
          class={[
            ...
            @tab != "sources" && "bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-400 hover:bg-gray-200 dark:hover:bg-zinc-700"
          ]}
        >
          Sources
        </button>
        <button
          ...
          class={[
            ...
            @tab != "url" && "bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-400 hover:bg-gray-200 dark:hover:bg-zinc-700"
          ]}
        >
          URL抓取
        </button>
        <button
          ...
          class={[
            ...
            @tab != "upload" && "bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-400 hover:bg-gray-200 dark:hover:bg-zinc-700"
          ]}
        >
          文件上传
        </button>
        <button
          ...
          class={[
            ...
            @tab != "manual" && "bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-400 hover:bg-gray-200 dark:hover:bg-zinc-700"
          ]}
        >
          手动添加
        </button>
        <button
          ...
          class={[
            ...
            @tab != "glossary" && "bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-400 hover:bg-gray-200 dark:hover:bg-zinc-700"
          ]}
        >
          Glossary
        </button>
          ...
          <div class="overflow-x-auto">
            <%= if @sources == [] do %>
              <div class="px-4 py-8 text-center">
                ...
                <p class="text-xs text-gray-300 dark:text-zinc-600 mt-1">通过 URL抓取、文件上传或手动添加来创建条目</p>
              </div>
            <% else %>
              <table class="w-full text-sm">
                <thead>
                  <tr class="border-b border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-950">
                    <th class="text-left px-4 py-2 font-medium text-gray-600 dark:text-zinc-400 text-xs">类型</th>
                    <th class="text-left px-4 py-2 font-medium text-gray-600 dark:text-zinc-400 text-xs">来源</th>
                    <th class="text-left px-4 py-2 font-medium text-gray-600 dark:text-zinc-400 text-xs">标题</th>
                    <th class="text-right px-4 py-2 font-medium text-gray-600 dark:text-zinc-400 text-xs">分块数</th>
                    <th class="text-right px-4 py-2 font-medium text-gray-600 dark:text-zinc-400 text-xs">操作</th>
                  </tr>
                </thead>
                <tbody>
                  <%= for src <- @sources do %>
                    <tr class="border-b border-gray-100 dark:border-zinc-800 hover:bg-gray-50 dark:hover:bg-zinc-900">
                      ...
                      <td class="px-4 py-2 text-right text-xs text-gray-500 dark:text-zinc-400">{src.chunk_count}</td>
      ...
      <%= if @tab == "url" do %>
        <div class="rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden rounded-tl-none">
          ...
          <div class="p-4 space-y-3">
            <p class="text-xs text-gray-500 dark:text-zinc-400">输入 URL 抓取网页内容作为 KB 条目。HTML 标签会被自动剥离，最多保留 5000 字符。</p>
            <%= if @url_flash do %>
              <div class={[
                "text-xs rounded px-3 py-1.5",
                String.contains?(@url_flash, "成功") && "text-green-700 dark:text-green-300 bg-green-50 dark:bg-green-950",
                String.contains?(@url_flash, "失败") && "text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-950"
              ]}>
                {@url_flash}
              </div>
            <% end %>
        ...
        <div class="rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden rounded-tl-none">
          ...
          <div class="p-4 space-y-3">
            ...
            <%= if @upload_flash do %>
              <div class={[
                "text-xs rounded px-3 py-1.5",
                String.contains?(@upload_flash, "失败") && "text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-950",
                !String.contains?(@upload_flash, "失败") && "text-green-700 dark:text-green-300 bg-green-50 dark:bg-green-950"
              ]}>
                {@upload_flash}
              </div>
            <% end %>
            ...
            <form phx-submit="upload_files" phx-change="validate_upload">
              ...
              <%= for entry <- @uploads.kb_file.entries do %>
                <div class="flex items-center gap-2 mt-2 text-sm">
                  ...
                  <span class="text-xs text-gray-400 dark:text-zinc-500">{round(entry.client_size / 1024)} KB</span>
              ...
              <%= for err <- upload_errors(@uploads.kb_file) do %>
                <div class="text-xs text-red-600 dark:text-red-400 mt-1">{upload_error_to_string(err)}</div>
              <% end %>
          ...
          <div class="p-4">
            ...
            <form phx-submit="manual_add" class="space-y-3">
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="text-xs font-medium text-gray-700 dark:text-zinc-300">条目 ID *</label>
                  <input
                    type="text"
                    name="id"
                    value={@manual_id}
            ...
            <%= if @glossary != nil && map_size(@glossary) > 0 do %>
              <div class="overflow-x-auto">
                <table class="w-full text-sm">
                  <thead>
                    <tr class="border-b border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-950">
                      <th class="text-left px-4 py-2 font-medium text-gray-600 dark:text-zinc-400 text-xs w-1/3">术语</th>
                      <th class="text-left px-4 py-2 font-medium text-gray-600 dark:text-zinc-400 text-xs">定义</th>
                      <th class="text-right px-4 py-2 font-medium text-gray-600 dark:text-zinc-400 text-xs w-24">操作</th>
                    </tr>
                  </thead>
                  <tbody>
                    <%= for {{term, definition}, idx} <- @glossary |> Map.to_list() |> Enum.with_index() do %>
                      <tr class="border-b border-gray-100 dark:border-zinc-800 hover:bg-gray-50 dark:hover:bg-zinc-900">
                        <td class="px-4 py-2 text-xs font-mono text-gray-800 dark:text-zinc-200">{term}</td>
                        <td class="px-4 py-2 text-xs text-gray-600 dark:text-zinc-400">{definition}</td>
                        <td class="px-4 py-2 text-right">
