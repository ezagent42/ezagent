      ...
      {:error, :cross_workspace_denied} ->
        {:noreply,
         put_flash(
           socket,
           :error,
           gettext("Cross-workspace denied — your workspace differs from this agent's workspace.")
         )}

      {:error, reason} ->
        {:noreply,
         put_flash(socket, :error, gettext("PTY input failed: %{reason}", reason: inspect(reason)))}
    end
  end
