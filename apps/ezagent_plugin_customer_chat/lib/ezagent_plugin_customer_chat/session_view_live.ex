defmodule EzagentPluginCustomerChat.SessionViewLive do
  ...
  # Durable mode read — on mount/refresh the operator UI must reflect
  # the session's REAL current mode, not a hardcoded `:auto`. We query
  # `Ezagent.Behavior.Mode`'s `:get` action (PR #511) which returns
  # `%{mode: :auto | :takeover}` from the session's `:mode` slice.
  #
  # Caller is `system://chat-router` (same system principal Mode itself
  # uses for its takeover notice) — this is a read-only system-side
  # query for display, and chat-router holds the authority to read any
  # session's mode regardless of operator cap provisioning. A failed /
  # missing dispatch falls back to `:auto` (fresh-session default).
  defp lookup_mode(%URI{} = session_uri) do
    inv = %Ezagent.Invocation{
      target: URI.new!("#{URI.to_string(session_uri)}?action=mode.get"),
      mode: :call,
      args: %{},
      ctx: %{
        caller: Ezagent.SystemPrincipal.uri("chat-router"),
