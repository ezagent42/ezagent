...
# Grant workspace-level session caps so founder can create+send in any session
ws_caps = [
  %Capability{kind: :workspace, behavior: Ezagent.ActionSet.Workspace, action: :create_session, instance: ws, workspace_uri: ws, granted_by: admin, granted_at: DateTime.utc_now()}
]
Enum.each(ws_caps, fn c ->
  {:ok, _} = Ezagent.Cap.issue({:admin, admin}, founder_uri, c)
end)
IO.puts("  workspace create_session cap granted to founder")
...
{:ok, signed_cap} = Ezagent.Cap.issue({:admin, admin}, admin, create_agent_cap)
...
# Join agent to session
agent_join_cap = %Capability{
  kind: :session, behavior: Ezagent.ActionSet.Session,
  action: :join, instance: g5_session,
  workspace_uri: ws, granted_by: admin, granted_at: DateTime.utc_now()
}
{:ok, signed_aj} = Ezagent.Cap.issue({:admin, admin}, agent_uri, agent_join_cap)
aj_result = Invocation.dispatch(%Invocation{
  target: EzUri.with_action(g5_session, :session, :join),
  mode: :call,
...
IO.puts("[4/5] Configuring registration...")
