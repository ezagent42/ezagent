defmodule Ezagent.Socialware.AnonBindingTest do
  ...
  describe "delete_expired/3 — the GC-safe compare-and-delete claim" do
    test "deletes a still-expired binding" do
      ttl = GC.ttl_ms()
      old = DateTime.add(@now, -49 * 3600, :second)
      {:ok, _} = AnonBinding.touch(@entity_a, @session, old)

      assert {:ok, :deleted} = AnonBinding.delete_expired(@entity_a, @now, ttl)
      assert AnonBinding.get(@entity_a) == nil
    end

    test "is a no-op (:not_found) when the candidate was refreshed since listing — the reap race" do
      ttl = GC.ttl_ms()
      old = DateTime.add(@now, -49 * 3600, :second)
      {:ok, _} = AnonBinding.touch(@entity_a, @session, old)

      # a concurrent public-route hit refreshes last_seen past the cutoff between the
      # GC's list_expired and its delete — the guarded delete must NOT reap it.
      {:ok, _} = AnonBinding.touch(@entity_a, @session, @now)

      assert {:ok, :not_found} = AnonBinding.delete_expired(@entity_a, @now, ttl)
      assert AnonBinding.get(@entity_a).last_seen_at == @now
    end

    test "is :not_found for an unknown entity" do
      assert {:ok, :not_found} = AnonBinding.delete_expired(@entity_b, @now, GC.ttl_ms())
    end
  end
