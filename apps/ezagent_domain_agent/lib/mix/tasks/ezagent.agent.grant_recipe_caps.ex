defmodule Mix.Tasks.Ezagent.Agent.GrantRecipeCaps do
  ...
  **boot-time, non-deliberate** entry — exactly what the
  ...
  #
  # The cc-headless default agents `pm-coordinator` + `dev-together` are NOT
  # here — they are no longer plugin `roles/0`; `ensure_all_started(:ezagent_domain_agent)`
  # above fires `Ezagent.Agent.DefaultRecipeSeed`, which seeds them so the lookup
  # below resolves. kanban STAYS (it owns the `kanban-manager` role via `roles/0`,
  # and loads `Ezagent.Behavior.Kanban` for the cap-resolve loaded-check).
