defmodule Ezagent.Agent.DefaultAgentSeed do
  ...
  This is the shared engine behind `Ezagent.Agent.DefaultRecipeSeed` (the generic
  boot seeder for the cc-headless default agents `pm-coordinator` + `dev-together`)
  and the kanban board-scoping `EzagentPluginKanban.PmCoordinatorSeed.materialize/4`.
  It lives in `ezagent_domain_agent` (NOT forked per-plugin) because it COMPOSES
  ...
  This is the SAME default `Ezagent.Agent.DefaultRecipes` derives for both pm and
  dev-together (absent a per-role `*_cwd` config override) — lifted here so the
