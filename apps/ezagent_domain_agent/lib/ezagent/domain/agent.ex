defmodule Ezagent.Domain.Agent do
  ...
  That path emits telemetry and records a durable DLQ obligation before invoking
  the VM lifecycle primitive.
  """
  @spec retire_spawned(URI.t(), URI.t() | nil, term(), keyword()) ::
          {:ok, map()} | {:partial, map()} | {:error, term()}
  def retire_spawned(agent_uri, provenance_root, reason, opts \\ [])

  def retire_spawned(%URI{} = agent_uri, %URI{} = provenance_root, reason, opts)
      when is_list(opts) do
    if Ezagent.AgentLineage.spawned_in_lineage?(agent_uri, provenance_root) do
      destroy_verified(agent_uri, reason)
    else
      maybe_destroy_unverified(agent_uri, provenance_root, reason, opts)
    end
  end

  def retire_spawned(%URI{} = agent_uri, nil, reason, opts) when is_list(opts),
    do: maybe_destroy_unverified(agent_uri, nil, reason, opts)

  defp destroy_verified(agent_uri, reason) do
    case Ezagent.Lifecycle.destroy(agent_uri, reason) do
      :ok ->
        :ok = Ezagent.AgentLineage.forget(agent_uri)
        {:ok, %{termination: :destroyed, provenance: :verified, cleanup_obligation: :none}}
  ...
  defp maybe_destroy_unverified(agent_uri, provenance_root, reason, opts) do
    if Keyword.get(opts, :allow_unverified_fallback, false) do
      obligation = %{
        target: URI.to_string(agent_uri),
        provenance_root: provenance_root && URI.to_string(provenance_root),
        retirement_reason: inspect(reason),
        obligation: "verify agent cleanup after unverified VM fallback"
      }

      :ok = Ezagent.DLQ.put(:agent_retirement_cleanup, obligation)

      :telemetry.execute(
        [:ezagent, :agent, :retirement, :unverified_fallback],
        %{count: 1},
        obligation
      )
      ...
      {:error, :unverified_spawn_provenance}
    end
  end
