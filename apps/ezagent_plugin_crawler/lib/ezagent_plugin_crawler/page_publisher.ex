defmodule EzagentPluginCrawler.PagePublisher do
  ...
  同一 seam，默认 `Ezagent.Invocation.dispatch/1`）。
  """
  ...
  @doc """
  读会话留存线索并发布 committed 页。零留存线索 → `{:ok, :empty}`（telemetry
  `:skipped`，不发空页）；否则走 `publish_items/3`。
  """
  @spec publish(URI.t(), String.t()) :: {:ok, String.t() | :empty} | {:error, term()}
  def publish(%URI{} = session_uri, summary) do
    case CrawlerRender.items_for(session_uri) do
      [] ->
