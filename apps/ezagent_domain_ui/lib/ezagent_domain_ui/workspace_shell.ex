defmodule EzagentDomainUi.WorkspaceShell do
  ...
  # 2026-06-16 — 右侧栏整体折叠(默认关闭;只有显式 opt-in 的页面才出折叠按钮,
  # 其它页面行为不变)。collapsed 状态由 host LiveView 管(server state → 重渲染不复位),
  # 折叠/展开按钮发 `toggle_right_sidebar` 事件,host 自己 handle。
