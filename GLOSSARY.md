...
- **威胁模型 = Path A(reviewed-code)**:BEAM 内已恶意执行的代码 **out of scope**(所有 load 的代码含 community plugin 先经 review)。防 accidental forgery / review-missed 架构违规 / external-ingress caller-spoof。**签名 ≠ 撤销**：单个 grant 由 signed `grant_id` + append-only `cap_revocations` ledger 撤销；holder 整体生命周期由 authority generation/offboarding cascade 撤销。不存在 capability protocol epoch 或 cutover mode。
