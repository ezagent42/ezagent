...
**接受的 scope(Phase-3)**:single-BEAM / trusted-node;crypto = **Phase-4**(`verify/1` 查 provenance format,非签名;in-VM 恶意 actor 超出 scope —— `:vm_internal` 信任模型已信任所有 in-VM code)。与 **Decision #154**(no unowned permissions)一致:`granted_by` = issuer 一定是 real entity;Decision #162。

参考: `.claude/skills/ezagent-developer/references/capbac.md` §4.5(current source of truth),Decision #154 #162,`docs/e2e/2026-07-11/phase3-cbac-done-right/`,invariant gates `apps/ezagent_core/test/invariants/cap_{self_store_paradigm_lock,issue_chokepoint,absorb_reachability,provenance_chokepoint,verify_load_boundaries,issued_bypass_trust_keys}_test.exs`
