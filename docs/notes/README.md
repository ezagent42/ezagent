...
- [Skill distribution to deployed agents (2026-07-08)](skill-distribution-design.md) — **[zh]** how agent SKILLS reach deployed agents, governed by the **recipe** layer (Allen: "由 recipe 去管理"). Root cause: `mix release` packages only `priv/`, so the repo-root `.claude/skills/` walk-up in `OrchestratorBootstrap` finds nothing in a release image → session-create dead on every channel (patched by ezagent-deploy `1d5aeca`, one hard-coded skill). Maps the (already-landed) `Recipe.skills` declaration + `RecipeRegistry`/`ConfigStore` three/four-state seed contract + np-uv and the socialware **single-source deploy-seed lane** (`priv/socialware_seed` → one-time `Home.SocialwareSeed` copy → single-dir scan) as reference models; the gap is a skill **content backend** (a `SkillRegistry` mirroring `RecipeRegistry`). Recommends the **socialware lane applied to skills**: `priv/skills_seed` (fresh image self-sufficient) → seed-once into `$EZAGENT_HOME/<profile>/skills/` (post-deploy add/upgrade, one runtime origin), content-hash reconcile with shipped-hash upgrade-vs-operator-edit discrimination. Authority explicit + phased: P1–P3 the store is system-vetted only (no runtime publish surface, unknown refs fail loudly); workspace-custom skills are a later phase via `resource://<ws>` + CR governance. Phased migration (P1 retires `1d5aeca` + generic resolver + subset DERIVED from `Recipe.skills` across `roles/0` seeds; P2 store+seed lane; P3 materialization folds into `HomeRuntime.stage_and_swap`, walk-up dies). Pushes back on async materialization (skill copy is KB text — sync). **Implementation SPEC** (codex handoff, P1–P3): [`../superpowers/specs/2026-07-08-skill-distribution-impl.md`](../superpowers/specs/2026-07-08-skill-distribution-impl.md) (**[zh]** twin).

- [cc-headless runtime: `claude -p` vs SDK persistent sidecar (2026-07-10)](cc-headless-runtime-claude-p-vs-sdk.md) — **quantified** comparison (measured in `ezagent-nightly`) + migration plan. Measured `-p` per-turn penalty over the SDK sidecar is **~1.5 s (no MCP) → ~1.9 s (with orchestrator MCP)** process startup; the model round-trip (2–11 s, ~2.6 s warm) is shared and its variance dwarfs the penalty. The current SDK worker hardcodes `setting_sources=[]`+`strict_mcp_config=True` so cc-headless has **no working MCP tools today**; `claude -p --mcp-config <orchestrator.mcp.json>` **loads and fires the orchestrator tools** (proven: `list_templates` invoked). Resource: SDK holds a resident `claude` node **~200 MB RSS + Python worker per agent at idle** vs `-p` **0 idle**. **Recommends `claude -p --mcp-config --resume` for both orchestrator and default agents** (no SDK inline-wiring build, reuses on-disk config the PTY path already generates); migration = new `HeadlessRuntime` module + `--resume` session storage, canary-gated on the real orchestrator firing a tool.
