...
- §1.2 关于"共用 authority 函数会导致 `cp_r` 泄漏"的论证在当前代码下不可达（真正的理由是别的）
- §8 的"非 admin 调用被拒"描述了一个**没有参数可查、也没有先例**的检查
- §1.1 说 `CapMint.mint/3` **写死** `kind: :agent, instance: agent_uri` —— **`mint/3` 本身是泛型的**（从第二个参数解构那两个轴），写死发生在它**唯一的生产调用点** `role_step.ex:194-200`。**这条是整支终审才抓到的，而且抓它的人指出：本记录当时列了三条、恰好漏掉第四条自己。**

**观察一**：四条都是**论证错了但结论对了**。设计文档里"为什么"那部分比"做什么"更容易写错，且更不容易被测试抓到 —— 它只会误导下一个读者。

**观察二（更值得警惕）**：第四条里，**代码注释是对的、设计文档是错的**。`agent_git_identity.ex:20-26` 的 moduledoc 由实现者写成了正确版本（"mint/3 itself takes kind/instance as parameters (it is generic), but its ONLY production call site … hardwires"），而设计文档一直是错的 —— **两者当面矛盾了整整五个 task 没人发现**。per-task 复审只看本 task 的 diff，不会去比对上游设计文档的论据段；只有整支终审逐条去读 `cap_mint.ex` 才抓出来。

**推论**：「设计文档的论据段」是一个**没有任何 gate 覆盖、且 per-task 复审结构上看不到**的盲区。它唯一的检查点是整支终审，而整支终审只跑一次。
