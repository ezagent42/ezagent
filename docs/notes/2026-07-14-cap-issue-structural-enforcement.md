...
> **每一个【模块名字面写在调用点上】的调用都被枚举了,且这一列的每一处赋值都被枚举了。**

**它没有证明** "每一个进入 `caps_json` 的 cap 都经过了 `Cap.issue`"。

(边界不是"静态可解析"—— 那词是错的:`mod = Ezagent.Users; mod.create(...)` 人眼一看就可解,但扫描器不做 dataflow,跟不过去。诚实的边界是**模块名有没有字面写在调用点上**。变量后面的模块名(`mod = Users` 或 `Module.concat`)一律抓不到,已在边界测试里钉成盲区。)
