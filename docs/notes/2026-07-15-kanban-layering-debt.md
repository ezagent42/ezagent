...
- **㊵ 记录(2026-07-17 r3)**:「分享到会话」气泡两处错——①文案「点击加入这块看板(按你的身份决定可编辑/可读)」不准:链接恒只读(H4),编辑权只来自成员身份,且同会话场景板已在列表无「加入」可言,应作「查看」;②点击接的是跨会话 HTTP receive 落点(服务端解析接收者目标会话→整屏重定向),同会话点击被带去别的 session。修法:气泡点击分流——板已挂在当前会话→原地切看板 tab 定位该板(零刷新);未挂→走 receive 挂载。文案改「查看看板(只读分享;会话成员按成员权限操作)」。归属:kanban 前端+receive 分流(我们,下批)。
- **㊵-模型钉死(2026-07-17 用户定):kanban 与会话正交**——a 在会话A建 k1;b 把 k1 分享到会话B;c 点击 → k1 出现在 **c 自己的 kanban tab**(只读),**不跳任何会话**。同会话分享同理:点击=跳当前 kanban tab 定位该板,gate 按点击者关系判只读/可写。⟹ 推翻 receive 落点「服务端解析目标会话+重定向」设计;**钥匙发给点击者本人**(person-scoped,非目标会话 assistant);kanban tab 内容=该用户持钥匙的板集合(人本位,正交于会话)。重塑分享二期与 D4 实现。归属:我们(receive 重做+tab 数据口径)。
- **㊶ 记录(2026-07-17 r3)**:kanban tab 不做屏幕适配——chat tab 会随窗口自适应布局,看板 tab 不会(疑固定宽/未用 flex 填充/溢出处理缺)。修法:看板容器与面板列改响应式(min-w/flex/overflow 对齐 Conversation 的布局骨架)。归属:kanban 前端(我们,下批)。
- **㊷ 记录(2026-07-17 r3复现)**:create_session 超时崩——world 建会话 dispatch ctx 无 deadline_ms,冷物化>5s 撞默认 GenServer 5s(报 {:create_session_exit,{:timeout,...5000}});底层会话多半已建成(幽灵成功,同⑨形)。**X 泛化:所有冷 provisioning 路(create_agent已修⑨/create_session本条/其余未查)都在裸奔 5s 默认——应系统性给 provisioning dispatch 显式 deadline(或统一 Provisioning 层默认 30s)**。修法:world 建会话 caller ctx 加 deadline_ms:30_000;顺手 grep 其余 provisioning call-site。归属:本 PR(小修)+X 泛化记给 Allen 线。
