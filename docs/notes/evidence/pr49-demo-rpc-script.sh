...
# `Ezagent.URI.new!/1` per SPEC 2026-05-27-uri-canonicalization §9.3
...
:erpc.call(:'$TARGET', fn ->
  ...
  seed_slice = %{
    ...
    agent_slots: [],
    orchestrator_template_uri: Ezagent.URI.new!(\"template://agent/cc-orchestrator\"),
    routing_rules: [],
    default_workspace_uri: Ezagent.URI.new!(\"workspace://pr49-demo\")
  }
  ...
  owner = Ezagent.URI.new!(\"user://admin\")
  ...
  workspace_uri = Ezagent.URI.new!(\"workspace://pr49-demo\")
  add_opts = [workspace_uri: workspace_uri, owner: owner]

  {:ok, backend_agent} =
    Tools.add_agent_slot(\"demo-backend-dev\", Ezagent.URI.new!(\"template://agent/cc-orchestrator\"), nil, add_opts)
  IO.puts(\"[3/6] add_agent_slot(:backend-dev) -> #{URI.to_string(backend_agent)}\")
  Process.sleep(1500)

  {:ok, reviewer_agent} =
    Tools.add_agent_slot(\"demo-reviewer\", Ezagent.URI.new!(\"template://agent/cc-orchestrator\"), nil, add_opts)
