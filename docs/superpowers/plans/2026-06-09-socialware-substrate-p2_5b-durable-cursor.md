# Socialware Substrate P2.5b — Durable Commit-Order Delivery Cursor Implementation Plan
...
defmodule EzagentCore.Repo.Migrations.SocialwareOutboxSurfaceVersionAndCommittedSeq do
  ...
  def up do
    ...
    # Backfill committed rows: assign committed_seq per session in commit order
    # (settlement.committed_at, then turn_id for ties) and copy surface_version
    # from the settlement's target_surface_version. Pending settlements (no
    # outbox visibility yet) keep committed_seq = NULL.
    flush()
    backfill_committed_rows()
  ...
  # Plain-SQL-free backfill via Ecto (portable). For each committed settlement
  # that has an outbox row, number them per session in (committed_at, turn_id)
  # order and set committed_seq + surface_version.
  defp backfill_committed_rows do
    repo = repo()
    ...
    rows =
      from(o in "socialware_customer_outbox",
        ...
        order_by: [asc: o.session_uri, asc: s.committed_at, asc: o.turn_id],
        select: %{turn_id: o.turn_id, session_uri: o.session_uri, target_surface_version: s.target_surface_version}
      )
      |> repo.all()

    rows
    |> Enum.group_by(& &1.session_uri)
    |> Enum.each(fn {_session, session_rows} ->
      session_rows
      |> Enum.with_index(1)
