...
> - **H3 (PR-3 Task 3.2) — DECIDED (Allen 2026-06-11), auto-apply preserved, no new entity:** at normal settlement, record the **manager URI** (the settling caller holding the agent's manage-cap) on the durable settlement state. In `turn.ex` `recovery_effects`, for the **config-apply replay only**, replace `SystemPrincipal.caps("bootstrap")` with `Ezagent.Identity.list_caps_for(manager_uri)` + `caller: manager_uri` — standard manage-cap gate re-validates (still holds → apply; lost it → denied, skip+log; `source_turn_id` idempotent). Mechanism already exists (`Identity.list_caps_for/1`; `recovery_effects` already uses the caller+caps shape with bootstrap). One-site change.
