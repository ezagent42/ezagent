...
- [ ] **Step 4: Write `mount_participation_caps` tests (failing)** — port `session_participation_caps_test.exs` from `e5b51888`; adjust tiers per spec §3.1 (unconfirmed = subscribe_from only; confirmed = +send+leave). Assert: (a) an unconfirmed member holds EXACTLY subscribe_from (no send); (b) a confirmed member holds send; (c) **FORGERY GUARD** — a join that REQUESTS the send/leave caps for an UNCONFIRMED recipient has those caps DENIED at the grant chokepoint (the recipient ends with only subscribe_from), proving the tier can't be forged via the trigger.

- [ ] **Step 5: Implement `mount_participation_caps/3` — FORGERY-SAFE design (SECURITY-CRITICAL, revised 2026-06-19).**

  **Constraint discovered:** `session.ex handle_join` carries a codex-reviewed security rule — authority-bearing facets must NEVER come from raw join args (a caller can forge them); they are caller-derived "as one reviewable security unit" (see the `:provenance`-deliberately-absent comment). The member's `confirmed` class is authority-relevant (it decides `:send`). So the tier MUST NOT be passed via join args (forge `confirmed:true` → `:send` on an anon = privilege escalation) AND must not be read inside `handle_join` (the 甲-1 sandbox/hot-path hazard).

  **Resolution — enforce tier eligibility at the TRUSTED grant chokepoint, not at the trigger:** the trigger (`handle_join`, `:async`/cast, like `e5b51888`) requests the **FULL** participation set (`subscribe_from` + `send` + `leave`) under `{:rule, :session_participation, owner}`. The IdentityAdmin grant chokepoint (trusted, DB-access) authorizes **per the recipient's TRUE `Users.confirmed?/1`**: `:subscribe_from` for ANY member; `:send`/`:leave` ONLY if `confirmed`. A forging caller can REQUEST send for an anon but the chokepoint DENIES it. This means `handle_join` needs NO class knowledge (no DB read, no forgeable arg) — it requests the full set; eligibility is filtered at the trusted layer. Implement the eligibility predicate in the `{:rule, :session_participation, …}` authorization path (`Ezagent.Identity.Grant` / IdentityAdmin), reading `Users.confirmed?(recipient)`.

  Run Step 4 tests → PASS (unconfirmed ends with EXACTLY `subscribe_from`; confirmed has `send`; a forged send-request for an anon is DENIED at the chokepoint).
