...
- [ ] **Step 4: Write `mount_participation_caps` tests (failing)** — port `session_participation_caps_test.exs` from `e5b51888`; tiers per spec §3.1. Assert: (a) an unconfirmed member, after join, holds EXACTLY `subscribe_from` (no `send`); (b) a confirmed member holds `send`+`leave`+`subscribe_from`. (No forgery-guard test — see Step 5: the tier is sourced from the authoritative DB, so there is nothing to forge.)

- [ ] **Step 5: Implement `mount_participation_caps/3` — BY-DESIGN-SECURE (caller-side, DB-sourced tier; revised 2026-06-19 per [[feedback_analyze_caller_before_defending]]).**

  **Threat-model first (the lesson):** the earlier "forge `confirmed:true` → escalate an anon to `:send`" hazard does **not** exist — anon users are created by INTERNAL code when a `public_view` session is accessed; the join/mount call chain is system-internal, never an external forger (Allen 2026-06-19). So **no chokepoint anti-forgery defense is needed.** By-design safety instead of defensive: source the tier from the authoritative DB (`Users.confirmed?`), never from caller args, so there is nothing to forge in the first place.

  **Design — caller-side mount (NOT in `handle_join`):** add `Membership.mount_participation_caps(session_uri, member_uri, owner_uri) :: :ok`. It resolves `Users.confirmed?(member_uri)` from the DB **at the caller layer** (the access points have DB access — avoids the 甲-1 handle_join DB-read hazard) and grants the tier per spec §3.1 (unconfirmed → `subscribe_from`; confirmed → `+send+leave`), `{:rule, :session_participation, owner}`, granter = owner. Because it runs in the CALLER (not the Session Kind), the grant→`Session.owner`→`get_slice` path is an ordinary cross-process call — **no self-deadlock, so `:sync` is fine** (cleaner than `e5b51888`'s forced `:cast`). Best-effort: log+telemetry on failure, never crash the caller.

  **Call sites (the trusted access points, after a successful join):** `session_context.do_maybe_self_join`, `admin/invite.ex`, `home_live.ex`, and the anon mint path (`anon_user.mint_for_public_session` / `chat_feed_controller`) so an anon also gets the `subscribe_from` read tier. Agents get `send` at spawn (甲-3), so the helper no-ops for agent members. `handle_join` is UNCHANGED (no mount, no class, no DB read).

  Run Step 4 tests → PASS (unconfirmed ends with EXACTLY `subscribe_from`; confirmed has `send`).
