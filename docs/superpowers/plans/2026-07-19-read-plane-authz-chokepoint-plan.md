...
| **tenant filesystem** (round-5 fix) | persisted layout/surface bytes read via `File.read` — `LayoutManager` (`layout_manager.ex:62`), addressed `resource://<ws>/world-layouts/<name>` | **`Ezagent.Resource.FsResolver`** — and note `LayoutManager.read_layout(scope_uri, scope)` is **already a MODEL chokepoint**: caller-scope threaded separately from target URI, `authority/2` rejects foreign-workspace, fail-closed (codex HIGH-4 hardened). A positive example of the Pillar-A contract. |

**IO surface is grep-closed (2026-07-19).** BEAM's local-persistence primitives are finite; every principal-tier content read bottoms out in one of: `Repo`/`Ecto` · `Kind.get_slice/*` · `KindRegistry` · content-caching `:ets`/`:persistent_term` · filesystem (`File` via `FsResolver`). Grep-confirmed **no `:mnesia`, no `:dets`**; static build assets (`plugin_asset_controller` `File.read` of compiled JS/CSS) are not tenant content → out of scope. External HTTP/socket reads are a different trust boundary (a named client module = store-owner, caught by Layer-2), not a local read-plane root. **The engine set is closed by exhaustion of the IO surface, not by enumeration luck** — a 6th "engine" would have to be a new BEAM persistence mechanism, which the semantic root definition already binds.
...
**Scope:** `InternalReads` for framework-internal reads (`Delivery.replay_messages_since/3`, reconcile/GC/boot) **plus** the internal-convergence session-slice reads currently in presenter/web tier (`uninstall_socialware`, `AnonTakeover.member?/verify_convergence` — `Kind.get_slice(_,:session)` reconcile): **split-then-relocate** the internal read into a framework-tier module behind `InternalReads` (a presenter module calling `InternalReads` is itself CI-red, criterion #12). **Recipe/template content plane:** `Ezagent.Agent.n/1,2`(`RecipeRegistry.lookup`) is the store-owner accessor over the recipe content-cache; its framework spawn/render callers (`role_step`, `cc_agent`, `definition_agents`, `kanban_render`, `skill_reconcile`) route via `InternalReads`; the principal-facing presenter read (`KanbanData.stages` → UI board config) routes through a workspace-authz chokepoint (PR-4) OR `InternalReads` if it's already board-view-gated upstream — the gate flags it; the implementer picks based on whether an upstream chokepoint already authorized the board view. Install the **complete** two-sided gate over the **full IO surface (5 engines) + content-store-owners**: (a) roots + store-owners reachable only from {chokepoints, store-owner, `InternalReads`} — incl. direct `:ets`/`:persistent_term` read of a content-cache (e.g. `:n`) outside `RecipeRegistry`, **and direct `File.read`/`File.stream` of tenant content outside `FsResolver`** → CI red; (b) `InternalReads` callers ⊆ framework tier. (`LayoutManager` already conforms — leave as the reference chokepoint.)
**Acceptance:** #7 full gate green across ALL planes — a direct `get_slice(_,:session|:surface)` / `DeliveryOutbox` / `Repo` / `KindRegistry` / `:ets.lookup(:n,…)` / `File.read`(tenant-content) read added to any presenter module → CI red (proves messages+members+delivery+surface+list+recipe+layout planes can't drift back); #11 arch acyclic green at §5 placements; #12 presenter module calling `InternalReads` → CI red. **Zero-orphan is PROVEN here structurally:** the full gate compiling green ⇒ no un-chokepointed principal read of any plane remains — no census required.
