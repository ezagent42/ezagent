# Git 凭据模型选型 — 四形态对比 + Plan A 三条 NO-GO 重核
...
四个候选形态，一张对比表，加上 Plan A 当年三条 NO-GO 的当日实测复核。本文**不**推进实现，也不修改任何运行时语义。
...
**每向右一格，都是以某几条后果换取功能。** 「不动」（停在 A0）是一个真实选项，且在安全维度上是最优的那个——代价是 §1.1 那一整串功能缺口。
...
| 维度 | **A0 今天** | A1 平台全驱动 | A2 受控 argv | B1 full-shell + token | B2 full-shell + SSH |
|---|---|---|---|---|---|
...
| 后果 | **A0 今天** | A1 平台全驱动 | A2 受控 argv | B1 full-shell+token | B2 full-shell+SSH |
|---|---|---|---|---|---|
...
**A0 是唯一四项全满分的形态**，代价是 §1.1 的全部功能缺口。任何向右的移动都必然放弃其中至少一项。
...
**关于停在 A0**：A0 在四条后果上全满分，是安全维度的最优解，且工时为零。但它无法支持私有仓库——这不是可以补的缺口，是 A0 的定义性质（§3 形态 A0 的关键推论）。若产品必须操作私有仓库，则**停在 A0 不是可选项**，问题只剩「向右移动几格」。
...
1. **形态**：A0（维持现状）/ A1 / A2 / B1 / B2 之一。注意：若产品必须支持私有仓库，A0 自动出局（§8）。
...
> **#NNN** — **Git 凭据模型定为 `<A1|A2|B1|B2>` + `<HTTPS token|SSH key>`**（2026-07-31，本 spec）。
...
> 接受的边界：`<按选定形态填写：凭据出现在何处、暴露给谁、窗口多长>`。
> 该边界**收口归统一隔离轨**，非本次范围。
