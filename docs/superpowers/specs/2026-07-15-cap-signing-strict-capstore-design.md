# Cap-signing: two symmetric chokepoints (verify + issue), isolated seed custody — DESIGN (v10)

**Status**: v10 — v9 review confirmed issuer-authentication is architecturally closeable (the three-source model is sound) and pinned the true final boundary: **seed custody**. Private Elixir functions do NOT close the signer bypass — the seed is env-readable (`System.get_env`), derivation is deterministic HKDF, and `:crypto.sign` is public, so any code can reproduce a valid signature off-chokepoint (and the verifier itself derives the private key from the same seed). v10 makes the master seed / private keys exclusive to an isolated signer; verification uses published public keys and never touches the seed. Pending re-review.
...
4. **Isolated seed custody — the real signer boundary.** The master signing seed and all private keys are held ONLY by an isolated signer (a dedicated process/service). Nothing else can read the seed or derive a private key. **Signing happens only inside the signer, only via `Cap.issue`, only after the issuer + issue-authority checks pass.** **Verification uses PUBLIC keys from a published keyring** (principal → pubkey), obtained without the seed. This is what actually makes "every cap passes `Cap.issue`" cryptographically true: without the seed, no code can produce a valid signature — an arch gate over function names cannot. Two chokepoints, both authenticating: `Cap.issue` gates who may mint; `authorize/3` gates who may use.
...
- **Isolated seed custody closes the bypass (not function privacy).** The signer bypass is cryptographic, not syntactic: the seed is `System.get_env`-readable, derivation is deterministic HKDF, and `:crypto.sign` is public — so hiding wrapper functions is theatre. The real fix: the **master seed + private keys live ONLY inside an isolated signer** (a dedicated process/service holding the seed in memory, never exposing it or any private key; the env seed is loaded once into the signer at boot and not left globally readable thereafter). The signer exposes only `sign(authenticated_issuer, cap)` — called only by `Cap.issue`, only after §7's issuer/issue-authority checks. **Verification changes too**: verifiers must use PUBLIC keys — the signer derives each principal's pubkey and publishes a keyring (principal → pubkey); `verify`/`authorize/3` read pubkeys from the keyring and NEVER derive from the seed (today's verifier deriving the full keypair from the seed is removed). Result: no code outside the signer holds the secret, so no valid signature can be produced off-chokepoint — a genuine cryptographic boundary, not an arch-gate.
