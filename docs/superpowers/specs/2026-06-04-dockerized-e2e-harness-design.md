...
- **Mutable test OAuth source — DURABLE shared mutable volume (codex r5 + r6):** `CredentialRefresh.provision/3` (PR-E) refreshes an *expired* source and **atomically writes the rotated token back to the source** (single-use refresh-token rotation keeps the source valid for the *next* run). A read-only source breaks the r3/r4/r5 fallback (cannot write back → unrecoverable). But a *per-run/disposable* writable copy is ALSO wrong (codex r6): the next run would re-copy the stale read-only seed, discarding the rotated token, so rotation fails after the first run — and concurrent runs would each lock their own copy while racing the same original refresh token. So the test OAuth source lives in a **durable mutable volume shared per profile/suite**, **seeded from the read-only mount only when absent** (first run), **never re-overwritten from the seed after rotation**; the OS lock file (`provision/3`'s cross-BEAM lock) is **colocated in that same durable volume** so concurrent runs serialize on the one rotating token. Per-agent creds are provisioned from that durable source into per-agent config dirs during scenario steps. (`feedback_self_generate_test_credentials`.)
...
**Step shape & hashing:**
- **`Ezagent.E2E.Step`** — `%Step{name, kind, inputs_hash, run, await, assert, cacheable?, valid_on_restore?}`.
...
**Restore-time validity predicates (codex r3 finding — time-sensitive state):**
- `inputs_hash` is a **creation-time** key: it proves the layer was produced by the current code. It says nothing about whether the layer's *contents* are still usable NOW. The clearest case: a layer captures the full `$EZAGENT_HOME` including **provisioned credentials**, which **expire over wall-clock time**. An fp-matching, assert-passed layer from yesterday can hold an expired claude/codex access token; selecting it skips the provision/refresh step → the scenario starts from stale auth and 401s (exactly tonight's live failure).
- So restore-validity is modeled as a **property of the layer's CONTENTS, checked GLOBALLY** — NOT delegated to the single step that happens to be selected. **(codex r4 finding:** a per-selected-step predicate is wrong — creds provisioned at step K live in EVERY layer M≥K, but selecting layer M would only run step_M's predicate, which has no cred check, so a later layer would restore expired creds and skip refresh.**)** Concretely:
  - Each layer's `manifest` records the **validity-relevant metadata of what is actually in its tar** — e.g. `creds: [{relpath, expires_at}]` for every credential file present (whichever earlier step provisioned/refreshed them). This is content-derived, so it is correct for any layer in the prefix that contains those files.
  - The resolver applies a set of **global restore validators** to EVERY candidate layer's manifest, independent of which step produced it. The credential validator = "every `creds[].expires_at` is in the future (minus skew)." Any candidate layer whose recorded creds are expired is rejected for selection.
  - A step's optional **`valid_on_restore?`** callback is the hook by which a step DECLARES extra content metadata to record + a global validator to register (the credential case is the built-in one); the resolver still applies it content-wise to every candidate, never as a single-step delegate.
  - On rejection the resolver falls back to an earlier qualifying layer (typically below the credential-provision step) and re-runs forward, re-provisioning fresh creds via `CredentialRefresh.provision/3` (refresh-if-expired). Credential-provision steps may alternatively be marked `cacheable?: false`; the content-based validator is the more efficient general form.
...
- **Resolver:** to run scenario up to target step T — compute `fp(0..T)`; find the **highest M ≤ T** with a cached layer whose `fp == fp(M)` **AND `assert_passed == true` AND `schema_version == current` AND all GLOBAL restore validators pass against layer M's manifest** (content-based, e.g. every recorded `creds[].expires_at` is in the future — checked for EVERY candidate regardless of which step produced it, §4.B); restore it (extract tar + `decode` ctx); run steps `M+1..T`. If a candidate fails only a restore validator (e.g. expired creds), the resolver **skips it and continues downward** to an earlier qualifying layer (typically below the credential-provision step), so the forward re-run refreshes the time-sensitive state. No qualifying layer → start from `base` (or blank) at step 0.
...
- **Resolver/LayerStore/Manifest unit tests** (highest-value pure logic), written test-first:
  ...
  - **expired-source re-provision** — the test OAuth source starts EXPIRED; after resolver fallback, provisioning refreshes it (writes the rotated token back to the durable source volume) and succeeds (codex r5 regression).
  - **cross-run + concurrent rotation** — two sequential fresh runs reuse the SAME durable source (the second sees the first's rotated token, NOT the stale seed); two concurrent runs serialize on the colocated lock and don't double-rotate (codex r6 regression).
