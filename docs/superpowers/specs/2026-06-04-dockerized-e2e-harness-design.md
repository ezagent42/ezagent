...
**(ii) Per-flavor credential sources (cc + codex) — durable mutable volume, shared per profile/suite:** the source holds the flavor's **entire `credential_relpaths` set** — cc: `.credentials.json` (OAuth, expiring); codex: a CODEX_HOME directory with `auth.json` (OAuth, expiring) **and** `config.toml` (static config). Because the OAuth entries expire and rotate single-use, the provisioner refreshes the *expired* OAuth entry and **atomically writes the rotated token back**, so the source is **writable + durable**: seeded from a read-only seed **only when absent** (first run), **never re-overwritten from the seed after rotation**. Each source's cross-process lock is **colocated** in its volume AND **crash-recoverable** — an **OS advisory lock on an open fd** (auto-released on process death) OR a lease with owner+mtime stale-lock recovery + fencing. (The current cc `provision/3` uses an `:exclusive` lockfile that is NOT crash-safe — the implementation MUST upgrade it; the codex provisioner is built crash-safe from the start.) **cc has its provisioner (#17 PR-E); codex's `refresh_test_credentials/3` is implemented WITHIN #21** (see §4.B + PR list).

**(iii) Per-agent credential dirs (cc + codex) — NEVER in a layer:** excluded from every layer tar (the FULL `credential_relpaths` set, §4.C) and re-provided on restore (§4.B). The provisioner's contract is to **reproduce EVERY `credential_relpaths` entry** for the flavor — *refresh* the expiring OAuth entries (cc `.credentials.json`, codex `auth.json`) AND *copy* the static entries (codex `config.toml`) — so a restored CODEX_HOME is complete, not just its token. (`refresh_test_credentials/3` is defined over the flavor's whole credential set, not a single file.)
...
- **(a) Flavors declaring `credential_relpaths`** (cc, codex): the WHOLE set is **excluded** from the layer and **re-provisionable** via `refresh_test_credentials/3`, which must reproduce EVERY entry — refreshing expiring OAuth entries and copying static ones (codex's `config.toml`). cc has its provisioner (#17 PR-E); **codex's is implemented within #21** (CODEX_HOME `auth.json` refresh + `config.toml` copy) so codex is layerable — NOT deferred.
...
- **Resolver/LayerStore/Manifest:** chain-hash; highest-valid-layer selection; pack/unpack round-trip **with the full per-flavor `credential_relpaths` set excluded** — the exclusion test ENUMERATES `CcAgent.credential_relpaths()` ∪ `CodexAgent.credential_relpaths()` (e.g. asserts NO `.credentials.json`, `auth.json`, OR `config.toml` in the tar), not hardcoded filenames.
...
- **PR-2b (codex provisioner):** implement codex `refresh_test_credentials/3` reproducing the FULL `credential_relpaths` set — CODEX_HOME `auth.json` refresh **and** `config.toml` copy — over a crash-safe durable per-flavor source, so codex joins cc as layerable (Allen: all three flavors). Mirrors cc's `CredentialRefresh`.
