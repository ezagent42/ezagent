...
- So a step may declare **`valid_on_restore?(layer_meta) -> bool`** — a **restore-time** predicate the resolver evaluates against a candidate layer at selection time (not creation time). The credential-provision step declares one that checks each credential file's `expiresAt` (minus skew) recorded in the manifest. If the predicate fails, the layer is treated as **invalid for selection**, so the resolver falls back to an earlier valid layer (before the provision step) and re-runs forward — re-provisioning fresh creds via `CredentialRefresh.provision/3` (refresh-if-expired). Equivalently, credential-provision steps may be marked `cacheable?: false` (always refresh on restore); the predicate is the more efficient general form.
- The manifest therefore also records the **expiry metadata** of any credential files in the layer (`creds: [{relpath, expires_at}]`), populated by the provision step.
...
- **Resolver:** to run scenario up to target step T — compute `fp(0..T)`; find the **highest M ≤ T** with a cached layer whose `fp == fp(M)` **AND `assert_passed == true` AND `schema_version == current` AND `step_M.valid_on_restore?(layer_meta)` holds NOW** (restore-time predicate, e.g. creds not expired); restore it (extract tar + `decode` ctx); run steps `M+1..T`. If a layer fails only the restore-time predicate, the resolver **skips it and continues downward** to an earlier qualifying layer (typically before the credential-provision step), so the forward re-run refreshes the time-sensitive state. No qualifying layer → start from `base` (or blank) at step 0.
...
- **Resolver/LayerStore/Manifest unit tests** (highest-value pure logic), written test-first:
  ...
  - **restore-time expiry rejection** — an otherwise fp-matching, assert-passed cached layer whose recorded credential `expires_at` is in the past is NOT selected; the resolver falls back below the provision step and re-provisions (codex r3 finding regression).
...
11. ~~Resolver selects credential-expired checkpoints~~ — **resolved**: `valid_on_restore?` restore-time predicates + manifest credential-expiry metadata; the resolver rejects expired layers and falls back below the provision step to re-provision (§4.B/§4.C). This also subsumes the #17 "auto-refresh on (re)spawn" gap *within the harness* — the provision step + restore predicate refresh deterministically.
