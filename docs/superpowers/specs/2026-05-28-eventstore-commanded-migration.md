...
> **DISPOSITION (2026-06-03, Allen): CONSIDERED — NOT ADOPTED.** ezagent did NOT migrate to EventStore/Commanded/full CQRS. We use the snapshot-based native state model (`Ezagent.Kind.Snapshot` / `SnapshotStore` / `StateRebuilder` in main), consistent with this spec's own r8 native-consolidation verdict. Merged as a design-of-record of the evaluation + decision, NOT as work to implement.
