...
- **OQ-6 (name clash) → DECIDED (Allen 2026-05-29): macro is `Ezagent.Lifecycle`; rename the existing `Ezagent.Behavior.Lifecycle` → `Ezagent.Behavior.Terminable`** on its migration. Naming journey (instructive — see §11 naming principles): `Lifecycle` (too generic + clashes with the macro) → `AdminControl` (rejected: too grand for a one-action module) → `TerminateWorker` (rejected: too NARROW + leaks the upper-layer "worker" concept) → **`Terminable`**. Verified the handler operates purely on `kind_module`/`supervisor/0` — it is fully **Kind-generic** (terminates ANY Kind's supervised process via dispatch+CapBAC; it only happens to be *registered on* the Agent Kind today). So the correct name is a Kind-layer capability name in the `Enumerable`/`Collectable` idiom — `Terminable` = "this Kind can be terminated through the gated path." It must NOT name `Agent` (an upper-layer composition concept that does not exist at the R/B/K layer). Under the migration it becomes `use Ezagent.Lifecycle` with a single `handle_terminate`.
...
## §11 — Naming principles (Allen 2026-05-29; carried into Phase B + ezagent-developer skill)

The `Lifecycle → AdminControl → TerminateWorker → Terminable` journey exposed a recurring failure mode: a small module gets a name that promises more (or names the wrong layer) than the code delivers. Root cause: **named by birth-context / host, not by responsibility + layer.** The original author was adding "a lifecycle action to the Agent Kind" and named the module after *what it attaches to* (`Lifecycle` on Agent), not *what it does* (terminate a Kind).

Three binding naming rules (added to the `ezagent-developer` skill + GLOSSARY + ARCHITECTURE.md, and enforced by a new lint):

- **NP-1 — Name by responsibility, at the narrowest accurate scope.** A module's name describes WHAT IT DOES, not what it attaches to or the PR that birthed it. A single-action Behavior's name must equal/closely track that action's intent. (`Terminable`, not `Lifecycle`.) Reinforces `feedback_self_explained_naming`.
- **NP-2 — Name in the vocabulary of the module's own layer.** A module in the R/B/K core layer (`ezagent_core`) may name only core-layer concepts (`Kind`, `Router`, `Behavior`, `effect`, `slice`/`state`). It must NOT name upper-layer composition concepts (`Agent`, `Session`, `Orchestrator`, `Workspace`, `Worker`) — those don't exist at this layer. (`Terminable`, not `AgentTermination`/`TerminateWorker`.) Capability-style names in the `Enumerable`/`Collectable` idiom fit core well.
- **NP-3 — Width match.** A module name whose semantic scope is clearly broader than its declared `action`s is a smell (over-promise); narrower-than-its-actions is also a smell (under-describes). Aim for equality.

**Lint (Phase B/C addition to `mix ezagent.check_invariants`):**
1. **Layer-vocabulary lint:** flag any module/file under `apps/ezagent_core/` whose module name contains an upper-layer concept word (`Agent|Session|Orchestrator|Workspace|Worker|Feishu|Cc|Codex|Np|Curl`). Allowlist genuine exceptions explicitly.
2. **Width lint:** for `use Ezagent.Lifecycle` modules, compare the module's final name segment against its declared action set; warn when the name is a generic lifecycle/admin/manager word but the module declares ≤1 action (the `Lifecycle`-with-one-`:terminate` pattern).

**Phase B rename audit:** while converting each of the 23 modules to `use Ezagent.Lifecycle`, the converting subagent ALSO checks the module name against NP-1/NP-2/NP-3 and records any violation. Confirmed renames so far: `Behavior.Lifecycle → Behavior.Terminable`. Any other violation found during conversion is reported (not silently renamed — a rename touches call sites + snapshot slice keys, so it needs the same care as `Terminable`).
