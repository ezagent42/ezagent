...
> 参考旧版 autoservice 流程：Master 创建空壳 → Tenant Admin 登录 → 自行初始化

```
阶段 1: Master 创建租户壳 (MasterDashboard → New Tenant)
┌─────────────────────────────────────────────┐
│ Master 只需提供:                              │
│   Tenant ID:  [demo-acme]                    │
│   Brand Name: [Acme Corp] (可选)             │
│                                             │
│ 创建后:                                       │
│   • workspace://demo-acme 创建               │
│   • admin 用户创建 (自动生成密码)              │
│   • 空 sandbox (soul/slots/skills/kb 均空)    │
│   • 无 CR，无 release                        │
...
└─────────────────────────────────────────────┘
                    │
                    ▼
阶段 2: Tenant Admin 登录 → 看到初始化引导
┌─────────────────────────────────────────────┐
│ Tenant Dashboard 检测到未初始化:               │
│                                             │
│  ┌─ ⚠️ 租户未初始化 ──────────────────────┐    │
│  │ 此租户尚未完成初始化配置。                │    │
│  │ 请运行初始化向导完成以下设置:              │    │
...
│ [████████████████░░░░░░░░░░░░░░░░░░░░░░░░]  33%                 │
│                                                                 │
│  填写以下信息，系统将基于模板自动生成 Soul:                        │
│                                                                 │
│  Brand Name:     [Acme Corp_______________]                     │
│  Industry:       [零售电商 ▾_____________]   ← 影响 L2 模板选择  │
│                  (通讯/零售电商/金融/教育/...)                    │
│  Service Hours:  [7×24小时_______________]                      │
│  Hotline:        [400-888-9999__________]                      │
│  Return Policy:  [7天无理由退换__________]                      │
│                                                                 │
│  ┌─ Soul 预览 (实时生成) ───────────────────────────────────┐   │
│  │ # {{brand_name}} 智能客服 Agent                          │   │
│  │ 你是 {{brand_name}} 的智能客服助手，                      │   │
...
```
┌─ 初始化向导 — Step 2/3: KB 知识库 ──────────────────────────────┐
│ [████████████████████████░░░░░░░░░░░░░░]  66%                   │
│                                                                 │
│  添加知识库内容 (可选，后续也可在 KB Manager 中添加):              │
│                                                                 │
│  ┌─ URL 抓取 ───────────────────────────────────────────────┐   │
│  │ [https://docs.example.com/faq_______]                    │   │
│  │ [+ 添加更多 URL]                          [开始抓取]      │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ┌─ 文件上传 ───────────────────────────────────────────────┐   │
│  │ [📁 拖拽文件到此处 或 点击选择]                            │   │
│  │ 支持: .pdf .txt .md .csv (最大 50MB)                     │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ┌─ 手动添加 ───────────────────────────────────────────────┐   │
│  │ ID: [kb-faq______]  Title: [常见问题____]                │   │
│  │ Content: [客户经常询问的问题包括...___________________]   │   │
│  │                                           [添加条目]     │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                 │
│                              [跳过]  [下一步: 预览发布 →]         │
...
│  │ Industry: 零售电商                                         │   │
│  │                                                            │   │
│  │ ✅ Soul: 已从模板生成 (customer.md, 85 lines, 2,048 bytes) │   │
│  │ ✅ Slots: 已预填 5 个值 (brand_name, industry, ...)       │   │
│  │ ✅ Skills: 3 platform skills inherited                    │   │
│  │ ✅ Fast Prompt: 已从 skeleton 复制                         │   │
│  │ ✅ KB: 0 URLs, 0 files, 2 manual entries                  │   │
│  │                                                            │   │
│  │ CR: cr-init-001 (draft)                                    │   │
│  │ Tracked Changes: 5 files                                   │   │
│  └────────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ┌─ Sandbox Preview ────────────────────────────────────────┐   │
│  │ (渲染后的完整 CLAUDE.md，含 slot 替换 + skill index)       │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ┌─ Lint ───────────────────────────────────────────────────┐   │
│  │ ✓ R01-R05 全部通过                                        │   │
...
[发布后]:
  → CrEngine.publish(tid) → v1 created
  → release/_current → v1/
  → Tenant status = "active"
  → Redirect to Tenant Dashboard (显示已发布状态)
```
...
```
Step 1 提交:
  ├─ 写 brand/industry 到 ConfigStore (TenantConfig)
  ├─ Soul Gen (基于模板 + 用户输入):
  │   ├─ 选择 L0-L3 模板: SoulLoader.load(priv_dir, tid, "customer")
  │   │    L0: agents/customer/soul.md (框架)
  │   │    L1: master/platform/customer.md (平台)
  │   │    L2: master/industry/<industry>/customer.md (行业)
  │   │    L3: sandbox/souls/customer.md (租户 ← 从 L0+L1+L2 合成)
  │   ├─ 提取 {{key}} 占位符: SoulSlotParser.parse_slots(L3_template)
  │   ├─ 写 slot 值到 sandbox/slots/customer.yaml
  │   └─ 写 soul 到 sandbox/souls/customer.md
  ├─ Skill 继承: 从 L0/L1/L2 复制 skill 引用 (只读，不拷贝文件)
  ├─ Fast Prompt: 复制 skeleton/config/fast_ack_prompt.md → sandbox/config/
  └─ CR: CrEngine.ensure_active_cr(tid) → 追踪所有初始化变更

Step 2 提交:
  ├─ URL 批量抓取: KbStore.fetch_url(kb_dir, url) × N
  ├─ 文件入库: KbStore.ingest_file(kb_dir, path) × N
  ├─ 手动条目: KbStore.upsert(kb_dir, entry) × N
  └─ CR 追踪: CrEngine.record_file_change(tid, "kb/sources")

Step 3 发布:
  ├─ Lint: CrLint.check(tid)
  ├─ Snapshot: CrSnapshot.snapshot(tid) → release/v1/
  ├─ Flip: update_current(tid, "v1")
  ├─ CR: status → "published"
  └─ Refresh: Refresh.refresh_agents(tid)
```

### 2.9 TenantOnboardLive — Master 创建租户壳
