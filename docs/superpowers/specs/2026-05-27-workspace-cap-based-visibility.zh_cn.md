...
| `apps/ezagent_domain_instance_message/lib/ezagent_domain_instance_message/application.ex:814` | `Ezagent.Workspace.Store.list_all()` | 不变 | agent flavor 解析需要扫每个 workspace 的 templates；非 operator 用面 |
...
`apps/ezagent_domain_instance_message/lib/ezagent_domain_instance_message/application.ex:273` —— 调用变为 `:ok = ensure_workspace("system", %{})`（不传 `visible: false`）。277-319 的 `ensure_workspace/2` helper 不用动 —— `attrs` 透传。
