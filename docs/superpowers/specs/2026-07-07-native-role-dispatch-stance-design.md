...
**Baseline:** every file:line claim in this spec re-verified at design time
against main `dcabf6174` (post #1208/#1212/#1213/#1215). T2 implementation
landed after T1 (#1219) and M2 (#1223), rebased onto main `df43253b2`.
...
This spec RELIES on T1 (`docs/spec-t1-materialize-behavior-fold`) landing first;
T1 is present on main as #1219 before this T2 implementation. The interface this
spec consumes, stated once:
...
### 4.3 Lead decision — default-responder policy

> **Decision: A — warn-only in Conformance.** When a Definition declares
> human-fillable roles but its merged rule set gives human free-text no
> adapter-flavor receiver, install remains allowed and Conformance emits a
> warning.
>
> Extend the orchestration spec's §3.2
...
> knowingly.
>
> **Rejected: B, reject install** on the same check. Fail-closed and consistent with
...
>
> **Future convergence: C, orchestrator socialware as default front desk.** The
...
> it. M2 has now landed, but this T2 check remains warn-only: it documents the
> mute-composite hazard without making the default front-desk rollout a hard
...
- **T-4 (cap fail-closed):** dispatch `builder.rebuild` without the cap →
  no generation (for `:cast`, the auth failure is recorded asynchronously);
  with the owner-granted cap → authorized and generation starts.
- **T-5 (⑦, §4.3 = A):** Definition with a human role and no adapter-flavor
  receiver for its traffic → `Conformance.check_with_warnings/2` emits the
  mute-composite warning while `Conformance.check/2` still returns `:ok`.
...
- **BV-1:** T1's landed interface name/shape for "recipe actions dispatchable
  post-materialization" — #1219 landed this interface before T2; §3.2's recipe
  wiring and T-1's setup consume that behavior.
