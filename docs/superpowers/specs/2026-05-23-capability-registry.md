...
| `apps/ezagent_domain_chat/lib/ezagent_domain_chat/application.ex:444-446` | Session `:send`, `:join`, `:leave` → Chat | `CapabilityRegistry.register(Session, :send, Chat)` etc. |
| `apps/ezagent_domain_chat/lib/.../application.ex:450` | Session `:set_working_copy` → Chat | direct migration |
| `apps/ezagent_domain_chat/lib/.../application.ex:451-452` | User `:receive`, Agent `:receive` → Chat (post-hoc) | direct migration — **per-Kind action subset preserved** (Chat exposes only `:receive` on User/Agent vs full 5 actions on Session) |
| `apps/ezagent_domain_chat/lib/.../application.ex:464` | Session Routing actions (loop) → RB | direct migration |
| `apps/ezagent_domain_chat/lib/.../application.ex:478` | Agent Pty actions (loop) → PtyB (post-hoc) | direct migration |
| `apps/ezagent_domain_chat/lib/.../application.ex:491-492` | AgentTemplate + SessionTemplate Template actions (loop) → TemplateB | direct migration |
| `apps/ezagent_domain_chat/lib/.../application.ex:508` | Agent Lifecycle actions (loop) → LifecycleB (post-hoc) | direct migration |
