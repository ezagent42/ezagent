...
- **P0 — Publisher as base behavior (TRUNK ONLY).** Add `Publisher.SessionImpl` to `SocialwareSession.behaviors/0` so every session composes the publisher **trunk** — a `:publisher` slice that exists and **records** every non-publisher slice change. **No consumer change, and no dispatchable publisher READ API on `SocialwareSession` yet** (no `@behaviour Ezagent.Behavior.Publisher` façade / `subscribe_from`/`snapshot`/`history`). The read API + its cap boundary are P3 (see below) — codex review of PR #711 found that adding the read façade in P0 (a) would dispatch `{:unknown_action, _}` (its read actions aren't registered) and (b) would let the broad shared `kind: :session` publisher cap read socialware INTERNAL slices, an authz widening. Gate: chat + socialware + Feishu scenarios unchanged; the `:publisher` slice records on every session.
...
- **P3 — ExternalAdapter (generalize ExternalMirror + fold CustomerFeed) + socialware publisher READ API + its cap boundary.** Define `Behavior.ExternalAdapter`; reimplement the Feishu mirror + the customer feed as adapters. This phase also wires the **socialware publisher READ API** (`snapshot`/`history`/`subscribe_from` dispatch on `SocialwareSession`) deferred from P0 — the adapter is its first consumer. **CRITICAL cap-boundary requirement (codex PR #711 HIGH):** the socialware publisher-read authz **MUST NOT reuse the broad `kind: :session` chat publisher cap.** Because `Session` and `SocialwareSession` share `type_name :session`, a single `kind: :session` publisher grant would let a chat/Feishu workspace publisher cap read `SocialwareSession`'s INTERNAL `:turns`/`:surface`/`:config_updates` slice payloads through the publisher ring — an authz widening from chat participation to socialware internal-state read. P3 must scope socialware publisher-read to the **concrete session/member/owner boundary** (per-instance / member-scoped cap), not the shared type-keyed publisher cap. The customer adapter's **source of truth is the durable committed-delivery outbox from P2.5, replayed by cursor/id** (committed-status gate + outbox idempotency preserved); the trunk event is the advisory wake-up that triggers a replay, never the sole signal. Gate: Feishu mirror E2E + customer SPA render/deny E2E both green on the new path, a **leak test** (no customer_visible data before commit), an **authz-boundary test** (a chat-only `kind: :session` publisher cap CANNOT read a `SocialwareSession`'s internal slices), AND the wake-up-loss test from P2.5.
