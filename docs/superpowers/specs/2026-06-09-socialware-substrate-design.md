...
- **P2.5 — Wire-schema regularization + a committed customer-delivery trunk event (subsumes #44; MUST precede P3).** Two coupled pieces codex flagged as prerequisites for P3:
  ...
  2. **Durable commit-signal via an outbox + cursor replay — NOT a best-effort slice-change event** (codex spec-review HIGH, rev3). A Publisher event produced via `SliceChange.emit` is **best-effort**: `SliceChange.emit` returns `:ok` and rescues/logs PubSub failures; Publisher appends its ring *asynchronously* on the PubSub `handle_signal`; the ring-persist in `kind/server.ex` is itself best-effort (catches DB failure, keeps in-memory). So a slice-change event **cannot** be the durable post-commit signal. Instead:
     - **The durable source of truth = the committed rows + an outbox**, written **transactionally as part of the settlement commit boundary** (extend the existing outbox/`status:committed` write), addressable by **cursor/id**.
     - **The trunk/PubSub event is only a best-effort WAKE-UP.** Losing it must NOT lose the delivery.
     - **The adapter is replay-from-durable-by-cursor + idempotent**: on wake-up OR on (re)connect it replays committed deliveries since its last cursor from the durable source. This generalizes today's `CustomerChannel` "refetch the gated snapshot on signal" (the snapshot already reads the durable committed rows) — the migration **preserves** that durable-source + advisory-wake-up shape; it does not replace the durable source with the best-effort trunk.
  - Gate: the existing `:customer_delivery` semantics (fires only post-commit, idempotent, committed-status gated) hold; AND a **wake-up-loss test** — drop the PubSub/trunk wake-up between `status:committed` and delivery, then prove the adapter still delivers (on reconnect / next wake-up) by replaying the durable source.
...
- **External adapters** — Feishu mirror (slice change → adapter publish); customer SPA **agent-browser visual E2E** (authenticated render of the json-render page + chat; unauthorized/cross-scope → denied) — the §36 standard, on an **isolated disposable seeded stack** (own ports, Tailscale `100.64.0.27`, never shared dev `:10042`/prod). PLUS two non-visual acceptance tests for the new path: a **leak test** (no `customer_visible` data delivered before `status:committed`) and a **wake-up-loss test** (drop the PubSub/trunk wake-up between commit and delivery → the adapter still delivers by replaying the durable outbox on reconnect/next wake-up).
