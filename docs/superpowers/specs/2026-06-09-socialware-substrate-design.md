...
- **Lifecycle (init slices / prune / reconcile / on_ready) iterates the INSTANCE's set**, not the module's.
- **Required/optional closure is resolved from the instance's set.**
- **Dispatch DENIES an action whose owning behavior is not in the instance's set — even if the module globally registers a superset** of actions on the one `SessionKind`. Likewise cap-checks are scoped to the instance's composed behaviors.

This is the precise contract that makes "chat is just a special socialware" safe: a chat instance (no `Surface`) must be *unable* to dispatch a `Surface` action, and must not init a `:surface` slice, even though the single `SessionKind` module knows about `Surface`.
...
- **P1 — Per-instance behavior set + required-closure resolver + instance-set runtime enforcement.** The load-bearing core (codex). Two coupled pieces:
  ...
  2. **Make the runtime instance-behavior-set-aware** (per §3.1 contract): persist the instance's behavior set; have lifecycle init/prune/reconcile/on_ready + slice init iterate the **instance** set; resolve closure from it; and make **dispatch + cap-check DENY actions whose behavior is not in the instance set** even if the `SessionKind` module registers a superset. This is what lets one Kind carry many apps safely.
  - Keep the two Kind modules for now (they become thin: each = a fixed instance behavior set); route everything through the per-instance path. Gate: the two current sets + the proposed chat/page sets all pass; a deliberately required-broken set fails loud in a test; **a denial test** — an instance WITHOUT `Surface` cannot dispatch a `Surface` action and does not init a `:surface` slice though the module knows `Surface`; valid sets unchanged at runtime (chat/socialware/Feishu scenarios green).
...
- **Runtime is module-static today** (dispatch `{kind_module, action}`, lifecycle iterates `behaviors_of(kind_module)`, caps per-module) → P1 must make it **instance-behavior-set-aware** (persist the set; lifecycle/dispatch/cap-check over the instance set; deny out-of-set actions) BEFORE one Kind carries many apps. Without this, a one-Kind superset registration would let any instance route any app's actions — a security/state-shape breach. This contract gates P5.
