...
>
> **rev 5 (AS-BUILT corrections from PR-2 implementation — these supersede H2 + a cap-flow detail):**
> - **H2 mechanism was infeasible as written** — the Lifecycle macro OWNS `post_init/2` + `handle_continue/3` (hard-coded `{:continue, :ezagent_activate}`, not overridable; the lifecycle gate hard-fails a developer `post_init`/`handle_continue`). The rev-4 `external_mirror.ex:73` citation was a STALE moduledoc. **AS-BUILT:** boot reconciliation uses `activate/2 → send(self(), :ezagent_ce_reconcile) → handle_signal/2 → dispatch(self, :reconcile_cascade)`. `activate` (pre-`:ready`, no siblings) only schedules; `handle_signal` self-dispatches the `reconcile_cascade` ACTION (the only path that builds full ctx with `ctx.siblings[:sandbox]`), carrying the agent's own caps.
> - **Deferred `sandbox.write_path` caps** — the deferred Cmd must carry the **agent's OWN** self-caps (read from `ctx.siblings[:identity][:caps]`), NOT `ctx.caps` (which in the apply handler is the MANAGER's manage-cap and does not authorize `Sandbox.write_path`). So `ConfigEvolve` declares `reads_siblings([:sandbox, :identity])`.
> - **Delta source** — the Agent has no `:turns` slice; the config delta is carried in the dispatch ARGS (layer/workspace_uri/subject_uri/key/patch, defaulting to the agent's own user layer), with `turn_id` as the idempotency marker. PR-3's Turn rewire forwards the settled delta's fields as those args.
> - Self-caps seeded in `Identity.create/1` for `:agent` URIs only.
>
...
**Recovery-path authority (H3 — DECIDED, Allen 2026-06-11; auto-apply preserved).** Auto-apply is kept (the Turn settlement still auto-triggers it; Allen: "需要自动apply"). NO new "evolver" entity is introduced — the authorizing identity is just the existing **manager** that holds the target agent's manage-cap. The `delta`'s `subject_uri` is available at settlement so step 1 targets the agent; the normal path carries the settling manager's caps. The hazard codex found: settlement **recovery** (`turn.ex` `recovery_effects`, ~:127-153) currently sets `ctx.caps = SystemPrincipal.caps("bootstrap")` (wildcard) for ALL replayed effects — laundering authority for the config replay. **FIX (mechanism already exists):**

1. At normal settlement, **record the manager URI** (the settling caller whose manage-cap gated step 1) on the durable settlement state alongside the config delta.
2. In `recovery_effects`, for the **config-apply replay specifically** (not the approve/commit effects), replace the bootstrap caps with the recorded manager's **currently-loaded** caps via `Ezagent.Identity.list_caps_for(manager_uri)`, and set `caller: manager_uri`. The standard manage-cap gate then re-validates against the manager's *current* authority: still holds it → apply; no longer holds it → denied by standard authz (skip + log; `source_turn_id` keeps a later fresh dispatch idempotent). NEVER bootstrap-launder the config replay.

The "dispatch as an entity" capability is already in the codebase — `Ezagent.Identity.list_caps_for/1` (used by `kind.ex:154,188`) loads any entity's caps by URI, and the dispatch ctx takes `caller`+`caps` explicitly; `recovery_effects` already uses exactly this shape (today with bootstrap). So the fix is a one-site change to the config replay's ctx.
