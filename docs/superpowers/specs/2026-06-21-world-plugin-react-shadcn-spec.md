...
- **⚠ HUMAN-ASSIST PREREQUISITE (not code, codex cannot do this)**: `world.ezagent.chat` must resolve — DNS + the Cloudflare tunnel front (prod is fronted by the `app.ezagent.chat` tunnel per `runtime.exs`). A code-only agent cannot provision DNS/tunnel. **Flag to Allen**: add the `world.` subdomain to DNS + the tunnel before prod verification. (Per [[feedback_flag_user_assist_steps]].)
