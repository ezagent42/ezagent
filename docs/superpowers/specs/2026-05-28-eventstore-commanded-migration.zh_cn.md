...
> **处置(2026-06-03,Allen):已考虑 —— 未采纳。** ezagent 未迁移到 EventStore/Commanded/完整 CQRS;采用 snapshot-based 原生状态模型(main 的 `Ezagent.Kind.Snapshot` / `SnapshotStore` / `StateRebuilder`),与本 spec r8 结论一致(原生整合)。合并为评估+决策的设计记录,而非待实现工作。
