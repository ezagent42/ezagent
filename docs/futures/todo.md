...
## 2026-07-24 — actor-extraction C0 boundary-gate hardening (follow-up) — ✅ DONE
...
Both follow-ups hardened in **PR #1549 "fix(actor): harden C0 boundary gate — interprocedural taint + reflective :sys"** (branch `feat/actor-boundary-gate-hardening`). Ledger delta = **zero** (forward 261, reverse 126 unchanged — codex's "zero current occurrence" confirmed empirically). `mix gate.arch` green. A RED self-test backs every listed evasion form.

- **[DONE] Interprocedural / access-path message-taint gaps.** Closed with a taint **fixpoint** over three sources (binding/alias, helper-return call-graph, interprocedural positional-parameter relay) plus access-path (`payload.m`) taint via `var_root/1`. RED self-tests: `closes interprocedural function-argument relay`, `closes map/tuple-field access-path taint`, `closes helper-chains >=2 deep`. Precision guard: the taint ORIGIN is Kind-message **construction** (`{:ezagent_*, …}` tuple), NOT any `:ezagent_*` atom — so config/ETS/app-name atoms (`:ezagent_domain_pty`, `:ezagent_role_registry`) never taint. Taint analysis is body-scoped (formal-parameter patterns in a head are not data flow). This precision was load-bearing: a first blunt pass surfaced 3 spurious `GenServer.call(:ezagent_*)` sites (`:status`, `{:write_input, …}`, `:__index__` — the PTY/feishu/session servers' OWN protocols) tainted by config atoms; tightening to construction-origin dropped all 3 → census back to 261 with no ledger growth.
- **[DONE] `:sys` reflective forms.** `apply(:sys, :get_state, [pid])`, `:erlang.apply(:sys, …)`, and `s = :sys; s.get_state(pid)` (variable receiver) now caught via dedicated `apply/3` + `:erlang.apply/3` + `:sys`-bound-var receiver clauses over the banned ops (`get_state`/`replace_state`/`get_status`). The `apply/2` capture form (`apply(&:sys.get_state/1, [pid])`) was already caught by the existing dotted `:sys` clause (verified by a self-test). RED self-test: `flags reflective :sys forms`.
- Backstop (retained): the per-phase codex adversarial review remains the diff-level check for interprocedural reach-ins the static gate could still miss.
