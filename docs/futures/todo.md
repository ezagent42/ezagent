...
Hardened in **PR #1549 "fix(actor): harden C0 boundary gate — interprocedural taint + reflective :sys"** (branch `feat/actor-boundary-gate-hardening`). **NET STRENGTHENING vs origin/main, never a weakening** — proven by an old-scanner-vs-new-scanner comparison: over origin/main's real tree the two are IDENTICAL (forward 239 unique / reverse 113 unique, 0 dropped, 0 added), and over a constructed battery the new scanner catches EVERYTHING old caught PLUS the newly-closed forms (0 "new-misses-old-catch"). Ledger delta = **zero** (forward 261, reverse 126 unchanged; `forward_new_offenders == []`, `forward_stale == []` → scanned set is identically the frozen ledger). `mix gate.arch` green.
