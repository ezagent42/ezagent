...
## 2026-07-24 — actor-extraction C0 boundary-gate hardening — ✅ common evasions closed (exotic sub-forms tracked below)

The `actor_internals_boundary_test` / `Ezagent.ActorBoundaryScanner` (landed in C0, PR #1546) is a **heuristic static AST reach-in detector**. codex's C0 adversarial review accepted it as sound for the merge (catches all reach-ins that actually occur + casual additions; site/SHA-anchored ratchet arithmetic verified) but constructed evasions with **zero current occurrence**, accepted as follow-up per the standing principle: *merge-blocking = a real bug / a reach-in that actually occurs / ratchet-integrity breakage; documented-follow-up = a constructed evasion of the heuristic with no current occurrence, backstopped by the per-phase codex review.*

Hardened in **PR #1549 "fix(actor): harden C0 boundary gate — interprocedural taint + reflective :sys"** (branch `feat/actor-boundary-gate-hardening`). **NET STRENGTHENING vs origin/main, never a weakening** — proven by an old-scanner-vs-new-scanner comparison: over origin/main's real tree the two are IDENTICAL for real Kind reach-ins (forward 236 unique / reverse 113 unique, 0 dropped, 0 added — the only forms the new scanner drops are the intentional config-atom false-positives, so precisely: *no old-caught real Kind reach-in is missed*), and over a constructed battery the new scanner catches every real form old caught PLUS the newly-closed forms (0 "new-misses-old-catch"). Ledger delta = **zero** vs the C1 base (frozen `@forward_frozen 255` / `@reverse_frozen 123`; scanner census forward 257 total / 236 unique, reverse 126 total / 113 unique; `forward_new_offenders == []`, `forward_stale == []` → scanned set is identically the frozen ledger). `mix gate.arch` green. (Census numbers are post-C1-rebase; before C1 they were forward 261 / 239-unique.)

> Correction (2026-07-24, after codex re-review of #1549): the first attempt narrowed the taint ORIGIN to Kind-message *tuple construction*, which WEAKENED detection origin/main had (missed an assigned bare atom `msg = :ezagent_get_slice` and a runtime-assembled tuple `List.to_tuple([:ezagent_get_slice, …])`). Fixed by switching the discriminator to a **protocol-verb allowlist** (`@kind_message_verbs`) instead — see below.

- **[DONE] Interprocedural message-taint gaps (relay, helper-chain ≥2).** Closed with a taint **fixpoint** over binding/alias chains, a helper-return **call-graph** fixpoint (helper-chains of any depth), and interprocedural positional-parameter relay (a tainted argument taints the callee's matching parameter). Body-scoped (formal-parameter patterns in a head are not data flow). RED self-tests: `closes interprocedural function-argument relay`, `closes helper-chains >=2 deep`, plus the regression guard `RETAINS origin/main's broad indirect detection`. **Precision discriminator:** the taint ORIGIN is the `@kind_message_verbs` protocol allowlist — the `:ezagent_*` atoms the actor GenServers actually send/receive (`:ezagent_get_slice`, `:ezagent_runtime_view`, …) — NOT the many other `ezagent_`-prefixed OTP-app / ETS / Registry / config atoms (`:ezagent_domain_pty`, `:ezagent_role_registry`, …). This both (a) preserves main's broad indirect detection (bare atoms, runtime-assembled tuples all carry a listed verb) and (b) excludes the config false positives — a first blunt pass had surfaced 3 spurious sites (`:status`, `{:write_input, …}`, `:__index__` — the feishu/PTY/session servers' OWN protocols, tainted by config atoms), all dropped by the allowlist. RED self-test for the precision: `does NOT taint on config/app-name :ezagent_* atoms`. The DIRECT `GenServer.call(pid, :ezagent_*)` arg check stays broad, so a direct send of ANY `:ezagent_*` message is flagged even for a not-yet-listed verb.
- **[DONE — common cases] Access-path / field-extraction taint.** `var_root/1` closes the common extractors: dot (`payload.m`, `a.b.c`), index (`payload[:m]`), `Map.fetch!/fetch/get(payload, k)`, `elem(payload, i)`; plus simple destructuring bindings (`{:box, msg} = payload`, `%{m: msg} = payload`). RED self-tests: `closes access-path field extraction`, `closes simple destructuring taint`. **STILL FOLLOW-UP (tracked, zero current occurrence):** interprocedural HEAD pattern-param destructuring (`def f(%{m: msg}), do: GenServer.call(p, msg)` — heads are body-excluded by design), deeply-nested/computed extractors, and container taint through opaque stdlib transforms.
- **[DONE — common cases] `:sys` reflective forms.** Caught via dedicated `apply/3`, `:erlang.apply/3`, and body-local `:sys`-bound variable-receiver (`s = :sys; s.get_state(pid)`) clauses over the banned ops (`get_state`/`replace_state`/`get_status`); the `apply/2` capture form (`apply(&:sys.get_state/1, …)`) is covered by the existing dotted `:sys` clause. RED self-test: `flags reflective :sys forms`. **STILL FOLLOW-UP (tracked, exotic, zero current occurrence):** `Kernel.apply(:sys, …)` (qualified), a variable op-name (`op = :get_state; apply(:sys, op, …)`), `:sys` relayed via a function parameter / module attribute, and `Function.capture(:sys, …)`.
- Backstop (retained): the per-phase codex adversarial review remains the diff-level check for interprocedural reach-ins the static gate could still miss (this is why the remaining exotic forms are tracked follow-up, not blocking).
