...
## 2026-07-24 — actor-extraction C0 boundary-gate hardening (follow-up)
...
- **Interprocedural / access-path message-taint gaps.** The `:ezagent_*` message-taint tracker misses: (a) function-argument relay — `msg = {:ezagent_*}; relay(pid, msg)` where `defp relay(p, m), do: GenServer.call(p, m)`; (b) map/tuple-field extraction — `payload = %{m: {:ezagent_*}}; GenServer.call(pid, payload.m)`; (c) helper-chains ≥2 deep. Harden with interprocedural param/return propagation + access-path taint (or an equivalent structural rule that does not bloat the ledger into "all dynamic GenServer.calls"). Add a RED self-test for each form.
- **`:sys` reflective forms.** `apply(:sys, :get_state, [pid])`, `:erlang.apply(:sys, …)`, and `s = :sys; s.get_state(pid)` are not caught (variable/reflective receiver). No current occurrence. Add reflective-receiver + `apply` coverage.
- Backstop: the per-phase codex adversarial review catches interprocedural reach-ins in each chunk's diff even where the static gate misses them — this is why the above are follow-up, not blocking.
