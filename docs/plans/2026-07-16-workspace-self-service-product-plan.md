...
| **兜底（GitHub Issue）** | 错误码未注册修复路径时 → 用户一键提 issue → 功能负责人排查 → 补充错误码注册 |
...
系统匹配错误码
    ...
    │   （有修复路径 + 用户有权限）
    │   例：founder 遇到 agent 缺凭证 → 跳转 Agent Keys 页
    │
    ├─ Layer 2: 需他人修复 ──→ 指名谁可以修 + 一键发送提醒
    │   （有修复路径 + 用户无权限）
    │   例：普通成员遇到 agent 缺凭证 → 「请联系 workspace founder <name>」
    │
    └─ Layer 3: 兜底 ──→ 一键提交 GitHub Issue
        （错误码未注册修复路径，或路径不可用）
        例：新上线的功能触发了一个尚未注册错误码的失败
```
...
**Step A1 — Agent 执行失败。** 用户发消息 → agent 操作失败（如缺凭证）→ 不走通用道歉。

**Step A2 — 结构化错误消息。** 系统匹配错误码，渲染消息卡片：
- **发生了什么**：「Agent 未配置 API 凭证」
- **影响**：「无法调用 AI 模型，你的消息暂时无法得到回复」
- **行动入口**：「[前往 Agent Keys 设置]()」（按钮/链接）

**Step A3 — 用户修复。** 点击链接 → 跳转修复页面 → 完成操作（如填入凭证）→ 系统即时校验 → 绿色 ✓ 确认。

**Step A4 — Agent 恢复。** Agent 状态自动更新为「就绪」→ 用户再次发消息 → 正常回复。
...
- **发生了什么**：「Agent 未配置 API 凭证」
- **影响**：「无法调用 AI 模型」
- **谁可以修**：「请联系 workspace founder 陈瑞华 配置 Agent 凭证」
- **行动入口**：「[发送提醒给陈瑞华]()」（按钮）

**Step B3 — 发送提醒。** 用户点击按钮 → 系统发送站内通知（+ 飞书/邮件，按 workspace 通知设置）→ 用户看到确认：「已通知陈瑞华」。
...
- 什么错误：「未配置 API 凭证」
- 谁发起的：「张三 请求修复」
- 行动入口：「[前往 Agent Keys 设置]()」

**Step B5 — Admin 修复。** Admin 点击通知中的链接 → 跳转修复页面 → 完成修复。

**Step B6 — 回执。** 修复完成后，发起提醒的用户张三收到回执：「陈瑞华 已修复 Agent 凭证配置」。

**Step B7 — Agent 恢复。** Agent 状态更新 → 张三再次发消息 → 正常回复。

---

**场景 C — 兜底：提交 Issue（Layer 3）**

**Step C1 — Agent 执行失败。** 系统匹配错误码 → 该错误码未注册修复路径（或已注册但路径不可用）。

**Step C2 — 兜底消息。** 消息卡片：
- **发生了什么**：「Agent 执行时遇到错误」+ 错误码（如有）
- **影响**：「无法完成你的请求」
- **行动入口**：「[提交问题报告]()」（按钮）

**Step C3 — 提交 Issue。** 用户点击按钮 → 系统预填 GitHub issue 模板：
- 标题：`[错误报告] <错误码> — <简短描述>`
- 正文预填：错误码、workspace ID、agent URI、时间戳、用户描述（用户自行补充）
- 用户编辑补充 → 提交
...
| Admin 动作 | 触发条件 | 界面/入口 |
|-----------|---------|----------|
| 收到修复提醒 | 有 workspace member 发起 Layer 2 提醒 | 通知中心（站内 + 飞书/邮件） |
| 查看错误详情 | 点击通知中的链接 | 跳转到对应修复页面（如 Agent Keys 页） |
| 执行修复 | 在修复页面完成操作 | 各修复页面（Agent Keys、Settings、成员管理等） |
...
**Layer 3（兜底 — 提交 Issue）**
- [ ] AC8: 错误码未注册修复路径时，消息卡片含「提交问题报告」按钮
- [ ] AC9: 点击按钮 → GitHub issue 预填错误码、workspace、时间戳，用户可补充描述后提交
- [ ] AC10: 提交 Issue 后用户收到确认：「问题报告已提交，我们会尽快处理」

**通用**
- [ ] AC11: 错误消息为中文（beta 范围）
- [ ] AC12: 冷启动实测：普通成员在 agent 失败时能理解发生了什么 + 知道下一步怎么做（自修 / 找谁 / 提 issue）
...
| C（兜底） | 😤 困惑 | → 虽然暂时修不了 → 但提了 issue → 有人会看 | 😐 至少不沉默 |
