...
| Feishu bindings → 正式 UserBinding dispatch (B2, handoff `feishu-binding-b2-world-dispatch`) | claude (`fix/feishu-world-binding-dispatch`) | `WorkspacePlugin.tsx` 的 FeishuBindings 局部（不动其余组件）、`workspace_plugin_data.ex` 的 Feishu clause、`workspace_plugin_actions.ex` 的 Feishu handlers、新 `Ezagent.World.FeishuBindingDispatch` helper、专属 tests | in-flight — 协调证据:registry 无既有 owner 覆盖这三个文件；全部 worktree 扫描确认无并发未提交改动；不动 `world_live.ex`/router/`main.tsx`/`styles.css`/shared primitives/slot registry |
