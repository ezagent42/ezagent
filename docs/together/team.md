...
| `gagameow` | gaga | human-dev | 黄佳佳 | 测试 W29 demo 路径（派活→平台 agent 出 PR→合→看板流转）交 allen 验收 + AgentRuntime 存量迁移 ARB-2~5 + LiveAuth/caps 审计（#1402 已合；⚠「EntityCaps 持久化统一」与 codex entity-caps D 重叠、待 lead 分派）；bridge-join 静默超时今日 esr-ng 无修复落地、待确认是否别处已关 | `#1375+#1379+#1381+#1402 (2026-07-14)` — 从空转 agent 查起滚出 3 已合安全 PR（PTY 归属 + create_user 提权洞修复 + cap-gate 诚实加固）+ 揪出 3 前所未见的安全洞；再落 #1402 AgentRuntime 边界门禁（只减不增 + 绕过对抗测试）+ canary 发现的 LiveAuth 权限可见性热修（改读 Ed25519 校验的 live Identity caps） | GMT+8 | verified |
...
| `allenwoods` | allen | lead | 林懿伦 | 验收 codex 两份返回（entity-caps A/B/D 实现 + cap-signing 无尾巴调查 findings）→ 定无尾巴 re-provision 升级实现 & enforce 时机（当前 dual-read、enforce 不翻直到 audit=0 未签名）；W29 demo 端到端集成粘合（Hello→Kanban 派活→平台 agent 出 PR→合→看板流转，本周至少跑通一次） | n/a | GMT+9 | verified |
...
| `ruihuachen-designer` | ruihua | designer | 陈瑞华 | 从产品角度完善 W29 demo（gaga 测→allen 验后接手打磨）+ #1378 rebase+解冲突→lead 合 + #1388 DealScout 撮合原型续（设计输入，不占 track 行） | `#1378/#1388 (2026-07-14, pending)`（官网目录重整 + flywheel design-brief + recruit-publish-flow；#1378 CONFLICTING 待 rebase、#1388 DealScout 原型 WIP） | GMT+8 | verified |
