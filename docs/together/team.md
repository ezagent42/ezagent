...
> **退出记录**：
> - `FatNine`（戴明，后端）2026-07-13 退出 ezagent 开发，已从 roster 与今日 track 移除。
> - `zhaomaota97`（张宁，全栈）2026-07-16 退出 ezagent 开发，已从 roster、go-live 账号表、profile 与今日 track 移除。email-server 邮箱 `zhang.ning@ezagent.chat` 已删除；canary/beta/stable 三环境用户账号 `entity://ezagent/user/zhang_ning` 待协调人执行 disable（见 offboard 报告）。
