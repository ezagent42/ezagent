...
| `allenwoods` | allen | lead | 林懿伦 | 裁定 #1376 方向（甲 `reconcile_session` 触发修增量-add-授权 X / 乙 mint_cap 作 #1360 挂载原语）+ #1360 Layer B 挂载 infra 排期；demo 集成粘合（hello live E2E + hello↔kanban 松耦合 + 看板流转 → 端到端本周至少跑通一次）；接收 gaga AgentRuntime SPEC；cbac Phase-4（crypto 签名——复用 `:crypto` + `cap.ex` 已留 issue/verify 接缝，登记） | n/a | GMT+9 | verified |
