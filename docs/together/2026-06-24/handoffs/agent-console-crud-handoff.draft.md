...
- [ ] **UPDATE** (primitive RESOLVED — `ConfigEvolve.apply_config_delta`, full cascade). The detail page renders the agent's **effective config cascade** (every config key — template defaults + any user-layer overrides, read from `ConfigStore`/the cascade) as **editable**. Edit any key (e.g. `advisor.behavior`, model, …), submit → a `{layer, key, patch}` delta dispatched through `ConfigEvolve.apply_config_delta` under the operator's manage-cap (`assert_subject_self` binds the subject to this agent). **Result:** the changed value is read back from `ConfigStore` and **persists across a full page reload** (re-fetch from the cascade, not an in-form echo). **Evidence:** two agent-browser screenshots — the new value shown *after reload* — plus a backend read (`iex`/CLI lookup of the config pointer/cascade) confirming the durable value matches. **Start with the full cascade exposed; trim keys later by UX feedback (Allen).**
