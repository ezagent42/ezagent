...
> **Status:** READY (lead decisions recorded 2026-06-24) — re-scoped to *Agent-contract CRUD only*. The load-bearing "what is Modify" question is RESOLVED: **Update = `ConfigEvolve.apply_config_delta` over the agent's FULL config cascade** (start full, trim by UX — Allen). Delete-gate, create-fields, and live-status decisions also recorded (§9). Assign to a dev.
...
- **Phase 0 — Update primitive CONFIRMED (Allen 2026-06-24): (b) `ConfigEvolve.apply_config_delta`, over the agent's FULL config cascade.** Not `Manage.:reconfigure` (unsupported until PR-5e), not a new domain action. Start by exposing the whole cascade as editable; trim keys later by UX. No further discussion gate — build it.
...
- **Phase 4 — Update (build-now; primitive confirmed).** An "Edit config" surface on the detail page renders the agent's **full effective config cascade** as editable; a change submits a `{layer, key, patch}` delta → `ConfigEvolve.apply_config_delta` under the operator's manage-cap; the changed value must **persist across reload** (the U acceptance point, §5). Render the whole cascade first; UX-driven trimming of which keys are editable is a follow-up, not a blocker.
...
- [ ] **UPDATE** (primitive RESOLVED — `ConfigEvolve.apply_config_delta`, full cascade). The detail page renders the agent's **effective config cascade** (every config key — template defaults + any user-layer overrides, read from `ConfigStore`/the cascade) as **editable**. Edit any key (e.g. `agent.soul`, model, …), submit → a `{layer, key, patch}` delta dispatched through `ConfigEvolve.apply_config_delta` under the operator's manage-cap (`assert_subject_self` binds the subject to this agent). **Result:** the changed value is read back from `ConfigStore` and **persists across a full page reload** (re-fetch from the cascade, not an in-form echo). **Evidence:** two agent-browser screenshots — the new value shown *after reload* — plus a backend read (`iex`/CLI lookup of the config pointer/cascade) confirming the durable value matches. **Start with the full cascade exposed; trim keys later by UX feedback (Allen).**
- [ ] **DELETE.** From the detail page, click Delete, confirm (a manage-cap holder; **Delete is blocked while the agent is bound to a live session** — the UI requires unbind first, per lead decision §9.2). **Result:** the agent is **gone from `agents_table` on reload** **AND** the **Kind process is actually terminated** (`Manage.:delete` schedules a *detached, async* `Lifecycle.destroy` — a vanished row alone is NOT proof). **Evidence:** agent-browser screenshot of `/identities/agents` **without** the deleted agent + a backend confirmation that the Kind is gone (`KindRegistry.lookup/1` returns not-found, or `mix ezagent` shows it absent). **Also** a delete attempt by a caller **without** the manage-cap showing the **cap-denial error** (no silent success).
...
- ~~The Update primitive~~ — **RESOLVED (§9.1): `ConfigEvolve.apply_config_delta`, full cascade.** Build it; no further gate.
- Any change to `AgentManifest` schema, `create_agent/3`'s accepted fields (to enable soul/skills/tools/executor — the #905 "Pending backend approval" set), CapBAC grant sites, or `Behavior.Manage`/`ConfigEvolve` semantics. All touch domain/core. (Note: using the EXISTING `apply_config_delta` is in-scope; CHANGING its semantics is not.)
...
**Lead decisions (Allen 2026-06-24 — RESOLVED, build to these):**
1. **(LOAD-BEARING) "Modify an agent" = (b) `ConfigEvolve.apply_config_delta` over the FULL config cascade.** Expose every config key (template defaults + user-layer overrides) as editable; each edit writes a `{layer, key, patch}` delta under the operator's manage-cap (`assert_subject_self`). NOT `Manage.:reconfigure` (unsupported until PR-5e), NOT a new domain action. **Start full; trim editable keys later by UX feedback.** This is the decision #904 lacked.
2. **Delete gate = manage-cap + confirm-dialog + block-if-bound.** A manage-cap holder with a confirm dialog, AND block delete while the agent is bound to a live session (require unbind first). (No pre-existing destroy-gate beyond manage-cap was found; this adds the bound-session guard.)
3. **Create author fields stay READ-ONLY** (status quo from #905). soul/skills/tools/lifecycle/executor are adjusted via Modify (cascade edit), not opened up in the create form. The `create_agent` extension is a separate, later decision.
4. **Live-status parsing: verify + fix in-scope (Phase 1).** Treat the `Phase/Flavor/Bridge: unknown` issue as in-scope — verify on `main`; if still broken, fix it here as part of R hardening.
