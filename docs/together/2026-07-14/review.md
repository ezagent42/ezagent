...
| **HomeLive fail-closed (auth escalation)** | #1403 — HomeLive no longer falls back to `entity://system/user/admin` on invalid/unresolvable identity (bad cookie → admin caps was a real escalation); clears session + redirects to login; SessionController parity; regression + arch anti-fallback invariant test. **On main, NOT yet deployed to canary (held: auth-path change).** | **gaga** |
| **bridge-join / agent-fault recovery SPEC** | #1405 — design-only (3 docs, 0 code); empirically **falsified the credential attribution** (creds-less claude still does full MCP initialize); Option B (remove session→agent edge) LOCKED; connects the bridge-join revoke-DLQ to entity-caps #1394 as the same silent-drop disease; **impl NOT LOCKABLE — 2 lead decisions pending** (#1394 unification substrate + cap-revoke option 1/2/3) | **gaga** |
...
- **bridge-join — attribution corrected (#1405), still not a demo blocker but has a real security dimension.** For the demo it's routed around by cc-headless (`:in_process_sync`, no bridge-join risk) and the acute create-timeout/@orchestrator-mute half was fixed (#1294, canary-proven #1367). BUT codex's #1405 investigation **empirically falsified the earlier credential attribution** (a creds-less claude still completes the full MCP initialize handshake — so "no creds ⇒ no bridge join" was wrong in 3 places); the real root is unknown and the design deliberately doesn't depend on it. Two real issues surfaced: (a) `ReadyGate.:failed` (a delivery gate) blocks `pty.write`/`pty.restart` (management actions) → a broken agent's creator can see the terminal but can't write/restart (2 of #1375's 3 perms are dead when most needed); (b) **cap-revoke is dead-lettered in prod TODAY** — one transport timeout drops a pending `:revoke_cap` into the DLQ → the cap stays. That's the same silent-drop disease as entity-caps #1394 gap A. #1405 is design-only, impl blocked on 2 lead decisions.
- **codex returns (update 2026-07-15):** the **cap-signing no-tail investigation returned** → coordinator verdict **NEEDS-REVISION (narrow): conclusions accepted, design phase can start, but the investigation test is red on re-run (non-hermetic, B1)**. entity-caps A/B/D still in flight. gaga's #1406 landed the capability-auth-followups umbrella; execution boundary set (codex = EntityCaps A/B/D; gaga = readers Tasks 3-6, gated on the facade landing; no-tail = separate ops). Enforce stays off until an audit proves 0 unsigned authorizer caps.
