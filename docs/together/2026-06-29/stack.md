...
| `returns/autoservice-live-verify-codex.md` | verify worktree, evidence only; related #1095 | codex | stacked -> partially closed by #1096 | The live create blocker is fixed by #1096. Remaining live issues are separate: Claude auth/bridge readiness, `Unsupported node: container`, pnpm Vite shim, anonymous channel write semantics. #1095 remains open as stale docs and needs refresh/close. |
...
| #1095 `docs(fp2): FP2 autoservice live verify` | open, mergeable=false, body still describes the old local-dispatch seam | Do not merge as-is. Refresh against #1096's final architecture or close as subsumed by #1096 plus `returns/autoservice-live-verify-codex.md`. |
