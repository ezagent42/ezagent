...
- Follow-up fix `08c8aaf9f59920ed97fe301403c19e0c9b8ee9e9` fixes the `session://system/default/feishu-bing` open failure class: World now rehydrates the target session, demand-spawns the viewer principal before JIT join, and patches unauthorized joins into observe-only conversation detail instead of leaving the operator on the Sessions table.
...
| 6 | Local build/architecture gates pass for the touched work. | met | Original FP4 gates passed: `mix assets.build`, `mix compile --warnings-as-errors`, `mix ezagent.arch.scan`, `mix ezagent.check_invariants`, and `curl http://127.0.0.1:10042/_health -> 200`. Navigation/session-open follow-up verification also passed locally: `node apps/ezagent_plugin_world/assets/test/world_navigation_test.mjs`, `MIX_ENV=test POSTGRES_PORT=5432 mix test apps/ezagent_plugin_world/test/assets/world_navigation_test.exs`, `MIX_ENV=test POSTGRES_PORT=5432 mix test apps/ezagent_core/test/invariants/uri_canonicalization_invariant_test.exs`, `MIX_ENV=test POSTGRES_PORT=5432 mix test apps/ezagent_core/test/invariants/plugin_workspace_locality_contract_test.exs`, `MIX_ENV=test POSTGRES_PORT=5432 mix test apps/ezagent_web/test/ezagent_web/world_host_routing_test.exs --trace` (17 tests, 0 failures after `08c8aaf9`), and `MIX_ENV=test POSTGRES_PORT=5432 mix test apps/ezagent_web/test/ezagent_web/world_conversation_test.exs --trace` (32 tests, 0 failures; emitted existing DB sandbox owner-exit log noise). |
...
| 8 | CI `precommit + check_invariants` is green on the PR head. | not-met | Implementation head `08c8aaf9f59920ed97fe301403c19e0c9b8ee9e9` fixes the World navigation URI parser gate, existing-member session-open regression, and the cold `feishu-bing` open class locally. Latest observed GitHub CI before this return-doc update was https://github.com/ezagent42/ezagent/actions/runs/28363802856/job/84024599620 on older PR head `af80077bc581693402097be556ec6f7a96704703`; it failed on `URI.parse/1` in `WorldLive` plus two `PluginIsolationWorkspaceTest` failures. Local `POSTGRES_PORT=5432 mix precommit` was attempted after `50c000a6` but failed on unrelated environment/baseline issues (`pg_dump` missing, DB schema workspace_uri checks, scanner timeouts, TTL boundary); lead should wait for post-push CI on the latest PR head. |
...
## Post-return update -- cold session open fix

- **updated_at:** 2026-06-29 19:20 +0800
- **Implementation commit:** `08c8aaf9f59920ed97fe301403c19e0c9b8ee9e9`
- **Behavior:** `SessionsTable` `Open` now prepares the runtime before `session.join`: `Ezagent.LocalRuntime.ensure_live(session_uri)` rehydrates durable-but-cold sessions, `SessionCreator.demand_spawn_member(caller)` makes the viewer principal live, and then the existing owner-rooted JIT join path can grant/authorize normally. If the viewer is not allowed to join, World still `push_patch`es to `/sessions?session=...` and the conversation renders observe-only.
- **Root cause of `session://system/default/feishu-bing` still not opening:** the previous fix covered a live session with a live existing member. The real baseline can be cold on either side: the session Kind may only exist in `kind_snapshots`, and the logged-in viewer principal may not be live when `Open` dispatches. `Membership.provision_join_authority/2` and `session.join` both rely on live Kind state, so the join grant/dispatch failed before any `push_patch` happened.
- **Local verification:** red/green was confirmed with new tests in `apps/ezagent_web/test/ezagent_web/world_host_routing_test.exs` for cold existing member, cold deep-link self-join, and observe-only unauthorized open. Passing commands: `MIX_ENV=test POSTGRES_PORT=5432 mix test apps/ezagent_web/test/ezagent_web/world_host_routing_test.exs --trace` (17 tests, 0 failures); `MIX_ENV=test POSTGRES_PORT=5432 mix test apps/ezagent_web/test/ezagent_web/world_conversation_test.exs --trace` (32 tests, 0 failures, existing sandbox owner-exit log noise); `MIX_ENV=test POSTGRES_PORT=5432 mix test apps/ezagent_plugin_world/test/assets/world_navigation_test.exs --trace`; `node apps/ezagent_plugin_world/assets/test/world_navigation_test.mjs`; `MIX_ENV=test POSTGRES_PORT=5432 mix test apps/ezagent_core/test/invariants/uri_canonicalization_invariant_test.exs --trace`; `MIX_ENV=test POSTGRES_PORT=5432 mix test apps/ezagent_core/test/invariants/plugin_workspace_locality_contract_test.exs --trace`; `mix format --check-formatted` on the three touched files; `git diff --check` on the three touched files.

## Gate status

- Implementation head: `08c8aaf9f59920ed97fe301403c19e0c9b8ee9e9`
...
- Failure summary: the previous WorldLive PID JSON failure is fixed; the next observed failure was stdlib `URI.parse/1` in `WorldLive` (fixed locally by `d1dd4062`) plus two `Ezagent.Integration.PluginIsolationWorkspaceTest` failures in the CI merge ref. Local targeted gates are green after `08c8aaf9`; local precommit also fails in this host on unrelated environment/baseline issues (`pg_dump` missing, DB schema workspace_uri checks, scanner timeouts, TTL boundary). Latest PR CI must be rechecked after push.
...
Do not merge yet. The branch is ready for review as the FP4 app-surface implementation plus World navigation reload fix plus cold session-open fix, but the machine return gate must be green on the latest PR head before merge.
