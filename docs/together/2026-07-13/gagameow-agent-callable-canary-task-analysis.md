...
2026-07-13 用户明确授权访问线上 canary；默认授权范围仍为**只读检查**：

- 允许：读取部署 SHA、容器状态/metadata、配置项存在性（只输出 `SET/MISSING`）、日志、Git 状态以及 HTTP GET 健康/产品读路径。
- 禁止：修改 `.env`/secrets、部署、重启/停止/重建容器、执行 migration/seed/reflow、写数据库、修改线上文件、改变账号/密码/权限、清理或创建线上状态。
- 产品验证本身需要创建 session、发送消息、触发 agent 或执行清理时，虽然属于既定 DoD，也属于线上状态修改；**执行每一类写行为前必须先向用户说明动作、原因与影响，并获得明确许可**。
- 若 subagent 需要访问线上服务，派发 prompt 必须原文携带本节约束；subagent 默认只读，不能自行扩大权限。
- 无法确认命令是否会写入时，按写操作处理并先请求许可。

授权增量记录：用户随后明确批准修正 Canary PAT pepper 注入位置并重建
Canary 应用容器，要求记录变更且保留精确回滚能力。该单次授权已执行并验证，
记录见本地忽略文件 `canary-deployment-location.md` 和脱敏证据
`docs/e2e/2026-07-13/agent-callable-canary/03-auth-recovery.txt`。此授权不自动
扩展到 session 创建、消息发送、agent 调用、清理或其他部署修改。
...
- 下一步需要任何线上写操作，即使它是正常产品操作或预期修复动作。
