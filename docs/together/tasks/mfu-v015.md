...
- **id**: `mfu-v015`
- **owner**: ruihua
- **status**: done（08-04 ruihua 回报: v0.15 续作四块内容均已合并完毕; 接棒卡 = `mvo-atlas`）
...
- [x] 认证移出树：N-07/N-11 → 认证徽章架（公司页 + 个人页）(evidence: 已合并, ruihua 08-04 回报)
- [x] 经验条 UI：两页上半区进度条（订单/打工/作品/人脉计数）(evidence: 已合并, ruihua 08-04 回报)
- [x] 外部认证录入入口（D41）(evidence: 已合并, ruihua 08-04 回报)
- [x] 目标 2：孵化器树 + 孵化器工作台（N-13 根 + grants 权限机制）(evidence: 已合并, ruihua 08-04 回报)
