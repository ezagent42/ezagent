...
- **待澄清**：登录后 Overview「Sessions」计数是否仍含它们、浏览/查看全部是否列出 —— 计数掉=真丢(持久化)，计数对但列表没=展示(#1280)。
- **处理**：新开 #1321，交叉引用 #1280(列表不一致)/#1257(冷启动持久列表)/#1279(create 超时)——「新建 session 不可靠出现」主线的一部分。
