...
- **已开 issue**：F1→#1279 · F2→#1280 · F3→#1281 · F6→#1282 · F7→#1284 · F4→#1285 · F5→#1286 · F9→#1289 · F10→#1290 · F11→#1291 · F15→#1303 · F19→#1313 · F21→#1315 · F23→#1321（F8→#1283 **已关，并入 #1288**；F12 **补入 #1274**；F13 **补入 #1272**；F14 **补入 #1291**；F16 **= #1281**；F17/F24 **补入 #1291**；F18/F20 **补入 #1280**；F22 **补入 #1290**）（关联 #1275 空 caps / #1278 agent 表单 / #1259 create 超时 / #396 agent onboarding）
...
### F14 · 高级规则编辑后点「添加」无反应（补入 #1291）
- **严重度**：中（0710 续测）
- **复现**：编辑高级规则 → 点「添加」→ **无反应**；预期 高级规则数量 +1、且新规则在某处显示，实际都没发生。
- **处理**：同 #1291（session 视图控件 silent no-op）一族 —— 疑 `session.routing.add` 静默失败，已补进 #1291。

### F15 · 会话列表「筛选会话/模板/状态」输入框点了不能输入（#1303）
- **严重度**：中（0710 续测）
- **复现**：左侧会话列表 → 点「筛选会话/模板/状态」文本框 → **无反应、不能聚焦/输入**，无法筛选会话列表。
- **线索**：与 #1291（session 视图控件）不同处 —— 这是会话**列表**侧的过滤框，独立开 #1303。

### F16 · 改应用设置时看不到「创建」按钮（= #1281）
- **严重度**：中高（0710 续测，与 F3 同一 bug）
- **复现**：新建会话左面板，应用选 Kanban 看板团队 → 成员配置很长、**无滚动条**、无鼠标滚轮 → **够不到「创建」按钮**。
- **处理**：**= F3 / #1281**（新建面板不滚动），已补 0710 repro；不另开。

### F17 · 成员面板点 agent 按钮不跳转（补入 #1291）
- **严重度**：中（0710 续测）
- **复现**：右侧成员面板 → 点 orchestrator 智能体旁的操作按钮（可见 tooltip「打开 PTY」）→ **无反应**：既没开 PTY、也没跳到 agent 设置页。
- **处理**：同 #1291（session 视图控件 no-op）一族，已补进 #1291。证据 `evidence/f17-member-panel-agent-button.png`。

### F18 · 新建的 session 没出现在左侧列表（补入 #1280）
- **严重度**：中（0710 续测）
- **复现**：新建 session **Test-Hello** → 中间会话区正常显示它(4 成员)、但**左侧会话列表里没有它**（只有 New-Test / Test）。正在看的新 session 不在列表里。
- **处理**：同 #1280（session 列表不一致）一族，已补 repro + 证据 `evidence/f18-new-session-not-in-list.png`。

### F19 · 绑定飞书外部镜像点 Bind 无反应（#1313）
- **严重度**：中（0710 续测）
...
### F21 · 点智能体「打开 PTY」终端按钮跳到了错的 session（#1315）
- **严重度**：中高（0710 续测，明确路由 bug）
- **复现**：在 /Default/Asdfasdfa（带终端智能体 zhang…）点该 agent 的「打开 PTY」→ **没开终端，反而跳到完全不同的 /Default/Hh**（Hh 只 1 成员、无终端 tab）。
- **处理**：即 #1291 里 F17 那个 PTY 按钮的**具体表现**（不是 no-op，是错跳 session）→ 单开 #1315，#1291 已指过去。证据 `evidence/f21-pty-before-asdfasdfa.png` / `f21-pty-after-wrong-session-hh.png`。

### F22 · 无法从成员面板移除智能体（补入 #1290）
- **严重度**：中（0710 续测）
- **复现**：成员面板里移除**智能体(agent)**成员 → 移不掉、agent 仍在。
- **处理**：即 #1290（移除成员 no-op）同一 bug，对象换成 agent —— 已补进 #1290（对 user 和 agent 成员都 no-op）。

### F23 · 重新登录后新建的 session 又没了（同一 workspace ezagent）（#1321）
- **严重度**：高（0710 续测，疑持久化/丢数据）
- **复现**：ezagent workspace 新建 session → 登出 → 重新登录 → 新建的 session **不见了**（确认还是同一 workspace ezagent，非切换误会）；已发生不止一次。
- **已确认 = 真丢数据**：重登后 Overview **Sessions 计数掉到 1**（原 6），用户建的 session 全没了、只剩种子会话。**不是展示问题**。
- **疑根**：新建 session **没真正落库** —— 很可能与 **create 5s 超时(#1279)** 同源（乐观创建/在服务节点建了，但超时前没 commit 到持久层，重登从 DB 重载即丢）。**高优**。
- **处理**：#1321，交叉引用 #1279/#1280/#1257。

### F24 · 点「加载更早消息」没用（补入 #1291）
- **严重度**：低-中（0710 续测）
- **复现**：点「加载更早消息」→ 无反应（`chat.load_older`）。
- **caveat**：若该会话本就没更早消息，no-op 可能是空态（非 bug）；但控件没给任何反馈。建议在真有分页历史的会话上验。已补进 #1291。
