...
- **Caps-cleanup batch (2026-05-25) — shipped**:
  ...
  - New Behaviors: `Ezagent.ActionSet.WorkspaceUserAdmin` (privileged `:create_user` carved from generic Workspace per PR #356 codex r1 CRIT), `Ezagent.ActionSet.UserCredentials` (set_password), `Ezagent.ActionSet.UserTokens` (mint/list/revoke), Feishu `UserBinding` + `SessionBinding` (per-Kind chokepoints from PR #355 case study).
