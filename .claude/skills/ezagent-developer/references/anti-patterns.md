...
## "I'll ship this socialware app as a seed script or one big integration branch"

Refuse as the long-term product shape. A socialware app must separate definition data, runtime substrate, and installer/deployment flow. Product content belongs in data carriers: AgentTemplate / soul markdown / resource fixture / SessionTemplate / socialware definition / ConfigObject / package metadata. Runtime substrate must be generic enough for multiple apps. The installer may import artifacts, ingest resources, grant caps through CapBAC, instantiate sessions/agents, and run verification. If AutoService lives as seed-owned persona/corpus/routing, or kanban lives as a branch mixing workflow data, substrate, UI, gateway, persona, and evidence docs, it is proving a path, not defining a mergeable product. See `docs/together/contributing/socialware-data-deployment-boundary.md`.
