...
> **2026-07-30 architecture recency (baseline `5af26bfd5`; the 26 principles all HOLD — this is *current state* you must not break):**
> - **Actor-framework extracted to its own app `apps/ezagent_actor` (#1579, C0-C7).** The actor RUNTIME (`KindRegistry`/`Router`/`ReadyGate`/`Kind.Server`/`Invocation`/`Kind.Snapshot`) LEFT `ezagent_core`; `ezagent_actor` has **zero in_umbrella deps** and core/domain/plugin now depend on `:ezagent_actor` via **config-injected ports** (`Ezagent.Kind.Adapters.wire!/0`; ports in `apps/ezagent_actor/lib/ezagent/kind/ports/*.ex`). **To read another Kind's slice use the public read surface `Ezagent.Kind.read/3` / `read_durable/2` — `get_slice`/`get_raw_slice` are DELETED.** A NEW **C0 actor-boundary gate** (`actor_boundary_scanner.ex`, interprocedural taint + reflective `:sys`, #1549) fails CI if non-framework code reaches actor internals; `Ezagent.Kind.Adapters` moduledoc saying "files still in core" is STALE.
> - **#189 identity-plane cutover landed.** `Ezagent.EntityCaps.Store` (domain_identity) is now the **authoritative durable identity-caps store** (status lifecycle `active｜revoked_unprovisioned｜tombstoned`); the durable **self-license** (`ActionSet.SelfLicense`) is issued specially and deliberately kept OUT of the store (`cap.ex:451`). **After any rebase, `mix ecto.reset` is mandatory before dev/e2e** (stale pre-cutover generation state boots `:holder_revoked` hard-fail).
> - **URI-share unified authz (A1-A2-2).** Sharing is the Feishu two-layer model: permission = cap ⊥ sharing = a per-target `Socialware.ShareSetting` toggle. `Ezagent.Cap.ShareToken` is a **stateless signed pointer carrying NO authority**; a claim mints NO durable cap — it records a `ShareClaim` and access is **re-derived LIVE** at the domain read gate via `Share.shared_to?/2` (disable is instant for everyone). Reverse index `EntityCaps.GranteeIndex.grantees_of/4` hangs on the **Store write throat, same-transaction**, and takes a **presented authenticated-caps witness** — never a free caller URI.
> - **owner-bypass debt → 0 (#1593).** Plugins/domain MUST route workspace-bound pid calls through `Ezagent.OwnerGatedExecutor.call/4` and session→workspace bind / agent-lineage record through `Ezagent.OwnerGatedWorkspace` — never raw `GenServer.call` on an agent pid or a direct `WorkspaceRegistry.bind`. The plugin-workspace-locality gate is now an **empty-allowlist enumerator** (RED on any remaining bypass).
> - **New CI gates/ratchets to respect:** `no_hardcoded_seed_principal` (#1560 — seed lane must not hardcode a principal; use `SystemPrincipal`), the C0 actor-boundary gate, `git_workflow` forbidden-claim-path (only `issue_for_action` is a sanctioned mint on that path; `Cap.issue/3`/`Capability.cap/5`/`Kind.spawn` gated out), PresenterCaps fresh-derivation. `mix ci.fast` still runs the fast subset locally.
> - **Umbrella is now 30 apps** (added `ezagent_actor` + `ezagent_plugin_git_workflow`); standalone `curl` Kind DELETED (folded to `Entity.Agent` flavor, #1623); the store substrate is fully Postgres (SQLite-era comments + `rebuild_from_events` retired, #1535).
