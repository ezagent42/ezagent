...
- **Local dev boot / `mix run` / e2e need `EZAGENT_SIGNING_SEED_V1` (Allen 2026-07-15, cap signing #1399)** — since Phase-4 ed25519 capability signing landed, any full-app boot (`mix phx.server`, `mix run <script>`, e2e seeds) requires `export EZAGENT_SIGNING_SEED_V1=<string ≥32 bytes>` (e.g. `$(openssl rand -hex 32)`), or `ezagent_plugin_world` fails at boot with `signing seed unavailable ... :missing_seed`. Dual-read + `require_signature:false` means a throwaway per-run seed is fine for dev/test (caps signed under an old seed still verify-fail soft). Isolated-env runbook: `docs/superpowers/handoffs/2026-07-14-cap-signing-notail-upgrade-codex-handoff.md`. Also keep the dev DB migrated (`mix ecto.migrate`) — a stale DB crash-loops boot on missing tables (e.g. `recipe_cap_bindings`).
