...
│                       #   (thin cards: each references its cross-day task file via `task: <id>`)
├── returns/<task>.md   # dev  (return):  timestamped done + DoD artifact + merge request
└── stack.md            # lead (push):    returns in analyzed merge order
```

**`docs/together/tasks/<task-id>.md` — FLAT, CROSS-DAY task files (2026-07-29;
supersedes per-day `handoffs/`).** Tasks roll across days, so the task record
does NOT live in a dated folder — a dated dir would fragment one task's history.
The flat task file is that task's SINGLE persistent record; each day's board
references it by id. Contents: `id` / 标题 / owner / **the full handoff (开工
prompt, written ONCE at task creation — never regenerated by later boards)** /
验收标准 / 依赖 / `started·est_done·actual` history / linked PRs·commits·codex
verdicts. **Cards are THIN status projections**: a board card carries
`task: <id>` + current status/dates; the thick substance (handoff, acceptance
detail, history) lives in the task file. The renderer folds the task file's
handoff into the card (collapsed), so a `done` card's 开工 prompt stays
