---
name: erlexec-elixir
description: Use when Elixir code runs OS processes with erlexec lifecycle management, including exec run/run_link/send/start/kill/stop/ospid/winsz/pty_opts calls, Ezagent.OSProcess or PyProcess, pty/os-process implementations, Port.open cleanup migrations, PTY streaming to xterm.js/web terminals/LiveView terminals, orphaned child debugging, or migration away from MuonTrap, naked Port.open, or tmux. erlexec is the sole OS-process and PTY library in ezagent. This skill captures verified API details that are easy to get wrong, including os_pid vs pid, monitor vs link, PTY options, winsz row/col order, sync vs async, and SIGKILL timing.
---
